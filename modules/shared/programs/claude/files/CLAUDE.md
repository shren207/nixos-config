...
세 스크립트 모두 `claude -p` 기반이라 ANTHROPIC_API_KEY 불필요. JSON을 stdout으로 출력, 진행 상황은 stderr.
...
- **`run_loop.py` Override**: `run-loop.sh`가 우선. 폴백으로 `python3 -m scripts.run_loop`도 가능하나 ANTHROPIC_API_KEY가 필요하다.
