...
agents_override() {
  ...
  auto_content+="- direct Codex 세션에서 review/audit/planning fan-out을 할 때는 nested \`codex exec\`보다 native subagent 경로를 우선 고려한다"$'\n'
  ...
  auto_content+="- repository-specific direct-Codex authority boundaries, write rules, and lock-sensitive command rules은 project-local skill docs 또는 \`AGENTS.override.md\` custom section에 둔다"$'\n'
