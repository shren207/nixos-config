...
- **이 가이드 읽고 바로 시작할 명령어** (복붙 즉시 실행 가능. 임의 cwd에서 실행해도 대상 repo로 복귀 + 실패 시 즉시 중단):
  ...
  - **실패 경로 격리 (서브쉘 + 명시적 `|| exit 1`)**: 모든 중간 명령(`gh repo clone`, `cd`, `ls-remote`/`fetch`, `git switch`, `git merge --ff-only`, 최종 `git status`/`git log`)에 `|| exit 1`을 부착하고 divergence/issue ref 부재는 명시적 `exit 1`로 처리한다. `set -e`는 POSIX 규정상 AND-OR list의 비최종 위치(`( ... ) || { ... }` 문맥 포함)에서 억제되어 사용하지 않는다. 상세 근거와 참고 링크는 `write-handoff/SKILL.md`의 "Handoff branch convention" 섹션 참조.
