...
- **NSS 복구 규칙**:
  ...
  2. remote branch가 있고 local branch가 없으면 `git switch --track "origin/$TARGET_BRANCH"`를 사용한다.
  3. local branch가 있으면 먼저 `git switch "$TARGET_BRANCH"`만 수행한다. dirty/ahead/diverged 상태에서는 자동 reset하지 않고 중단한다. clean + behind 상태에서만 `git merge --ff-only "origin/$TARGET_BRANCH"`를 허용한다.
  4. remote branch가 truly absent(`ls-remote` no-match)이고 local branch만 있으면 그때만 `git switch "$TARGET_BRANCH"` fallback을 허용한다.
  5. transport error, repo/branch 미확정, ref 부재는 모두 fail-closed다. 기본 branch로 자동 복귀하지 않는다.
- **실패 경로**: `||` 에러 블록이 `ERROR: handoff restore failed. REPO=... ISSUE_NUM=... TARGET_BRANCH=...`를 출력하여 재시도 안내만 한다.
...
**cross-repo linked branch/PR 제한** (`[UNVERIFIED]` 현재 미지원): GitHub는 issue를 다른 repository의 branch 또는 PR과 연결할 수 있다 ([linked branch 문서](https://docs.github.com/en/issues/tracking-your-work-with-issues/using-issues/creating-a-branch-for-an-issue), [linked PR 문서](https://docs.github.com/en/issues/tracking-your-work-with-issues/using-issues/linking-a-pull-request-to-an-issue)). 현재 이 스킬은 `linkedBranches.nodes[0].ref.name`과 PR `headRefName`만 읽고 `repository { nameWithOwner }` 차원은 저장하지 않는다. `TARGET_BRANCH`가 issue repo와 다른 repo에 있는 경우:
...
}' -f o="$OWNER" -f r="$REPO_NAME" -F n="$ISSUE_NUM" \
  -q '.data.repository.issue.linkedBranches.nodes[0].ref.name'

PR_NUM="$(gh issue view "$ISSUE_NUM" --repo "$REPO" --json closedByPullRequestsReferences \
...
`gh issue view --json`은 `linkedBranches`를 지원하지 않으므로 해당 조회는 반드시 `gh api graphql` 경로를 사용한다.
...
3. `TARGET_BRANCH`가 빈 문자열, `null` 리터럴 문자열, placeholder 형태(`<...>`)이거나 repo-local heuristic에서 온 후보값뿐이면 Step 1-C를 재실행한다. 여전히 미확정이면 질문 도구로 사용자에게 exact branch를 확답받는다.
4. **NSS 블록에 unresolved placeholder(`<REPO_SLUG>`, `<ISSUE_NUM>`, `<TARGET_BRANCH>`, `<unknown-*>`, 빈 문자열)가 남은 상태로는 Step 9(게시)로 진행하지 않는다.** Step 8 self-verification에서 placeholder 잔존 여부를 재검증한다.

**Shell-안전성 가드 (필수)**: NSS 블록은 `REPO='...'` / `ISSUE_NUM='...'` / `TARGET_BRANCH='...'` 처럼 single-quoted literal로 값을 삽입하여 `$(...)`, 백틱, `$var` 해석을 차단한다. REPO 또는 TARGET_BRANCH 값에 **`'`(single quote)** 가 포함되면 single-quoted literal을 조기에 닫아 주변 명령을 오염시킬 수 있으므로 치환 즉시 게시를 중단하고 질문 도구로 사용자에게 escape 전략을 확답받는다. (ISSUE_NUM은 정수 이슈 번호이므로 single quote가 포함될 수 없다. `$`/`"`/백틱은 single-quoted literal 내부에서는 리터럴로 처리되어 안전하다.)

Next Session Starter 블록 작성 시 위에서 확보한 REPO, ISSUE_NUM, TARGET_BRANCH 세 값을 실제 값으로 치환해 handoff 본문에 포함한다. 예시 template의 repo slug(`greenheadHQ/nixos-config` 등) 하드코딩이나 `issue/$ISSUE_NUM` 재구성값을 그대로 두지 않는다.
...
5. **NSS placeholder 검증 (필수)**: Next Session Starter 블록의 REPO, ISSUE_NUM, TARGET_BRANCH가 다음 중 하나라도 해당하면 Step 1-D "값 확보 실패 처리" 순서로 실제 값 확보 후 치환. 치환 완료 전에는 Step 9(게시)로 진행하지 않는다.
    - `<...>` 형태 placeholder (`<REPO_SLUG>`, `<ISSUE_NUM>`, `<TARGET_BRANCH>`, `<unknown-*>`, `<repo-root-path>` 등)
    - 빈 문자열
    - `null` 리터럴 문자열
    ...
    - **TARGET_BRANCH 값이 Step 1-C에서 확정한 branch와 불일치**: repo-local heuristic가 남아 있거나 `issue/$ISSUE_NUM` 같은 재구성값이 남아 있으면 반드시 치환한다. heuristic candidate를 사용했다면 Step 1-C 기준(actual branch 확보 또는 사용자 확답)으로 재검증한다.
6. **Shell-안전성 재검증 (필수)**: NSS 블록의 `REPO=`와 `TARGET_BRANCH=` 할당이 **single-quoted literal(`'...'`)** 형태인지 확인하고, 두 값 중 하나라도 `'`(single quote)을 포함하면 Step 9를 중단한다 (Step 1-D Shell-안전성 가드 참조). ISSUE_NUM은 정수이므로 single quote 위험이 원천 없다.
