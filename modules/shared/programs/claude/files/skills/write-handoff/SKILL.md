...
| `ERR_GH_UNKNOWN` | 분류 실패 | 코드(`ERR_GH_UNKNOWN`)만 사용자에게 보고한다. helper는 raw `gh` stderr를 의도적으로 emit하지 않는다(URL/credential/local path 누출 차단). 디버깅이 필요하면 사용자가 helper를 같은 인자로 직접 재실행하여 stderr를 인터랙티브하게 검사한다 |

stderr가 비어 있는데 `REPO_SLUG`/`ISSUE_NUM`이 빈 문자열, `null` 리터럴 문자열, placeholder 형태(`<...>`)이면 helper 버그 가능성 — 그대로 사용자에게 보고한다.

위 표는 control flow의 단일 진실이다. `ERR_AUTH`(중단), `ERR_NETWORK`(재시도 후 보고), `ERR_GH_UNKNOWN`(코드만 보고)은 **terminal**이며 URL 재실행을 시도하지 않는다 — 인증/네트워크가 막힌 상태에서 같은 호출을 반복해도 결과가 같다.

값 재확보가 가능한 실패 경로(`ERR_NOT_FOUND`, `ERR_URL_PARSE`, `ERR_NO_CWD_REPO`, 또는 stderr가 비어 있는데 출력이 빈 문자열·`null`·placeholder인 invalid-output 케이스)에서는 다음 순서로 재시도한다:
