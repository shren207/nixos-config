...
**자율 재개 범위**: handoff는 REPO 복귀와 작업 맥락(Phase/재개 지점/산출물 위치)을 자율 재개용으로 보장한다. **작업 branch 선택은 실행 시점에 사용자 지시로 결정**한다 (handoff 작성 시점의 branch 추정은 stale 위험이 구조적이므로 이관). NSS는 repo 복귀 명령 + 선택적 `<WORKING_BRANCH>` 후보 bullet을 제공하고, 실행자가 후보를 참고하여 checkout한다.
...
3. 확신이 없으면 런타임 도구 매핑 표의 질문 도구로 **이슈 URL 또는 repo slug**를 사용자에게 명시적으로 확답받는다. 이슈 URL을 받으면 helper를 URL 인자로 재실행. repo slug(`owner/name`)를 받으면 helper를 건너뛰고 그 값을 직접 REPO로 사용한다 (helper는 `gh issue view "$arg"`로만 처리하여 repo slug를 해석하지 못함).

bare 번호 + cwd 불일치 조합은 Step 1-C 실패 처리 대상이다.

**1-C. 값 확보 실패 / 잘못된 값 처리**

다음 중 하나에 해당하면 이 절차를 수행한다:
- 1-B 결과가 **빈 문자열** 또는 **placeholder 형태(`<...>`)**
- 1-B 결과가 non-empty이지만 **bare 번호 + cwd 불일치로 얻은 wrong slug**(위 "bare 번호 입력 시 cwd 의존" 단락 참조)

처리 절차:
1. helper를 재실행한다. bare 번호 + cwd 불일치의 경우 사용자에게 **이슈 URL 또는 repo slug**를 확답받은 뒤:
   - 이슈 URL을 받으면 helper를 **URL 인자**로 재실행.
   - repo slug(`owner/name`)를 받으면 **helper를 건너뛰고 그 값을 직접 REPO로 사용**한다 (helper는 repo slug를 해석하지 못함).
2. 여전히 실패하면 런타임 도구 매핑 표의 질문 도구로 사용자에게 repo slug를 확답받아 직접 REPO로 사용한다.
3. **NSS 블록에 unresolved placeholder(`<REPO_SLUG>`, `<unknown-*>`, 빈 문자열)가 남은 상태로는 Step 9(게시)로 진행하지 않는다.** Step 8 self-verification에서 placeholder 잔존 여부를 재검증한다.

**Shell-안전성 가드 (필수)**: NSS 블록은 `REPO='...'` 처럼 single-quoted literal로 값을 삽입하여 `$(...)`, 백틱, `$var` 해석을 차단한다. 확보한 값에 **`'`(single quote)** 가 포함되면 single-quoted literal을 조기에 닫아 주변 명령을 오염시킬 수 있으므로 치환 즉시 게시를 중단하고 질문 도구로 사용자에게 escape 전략을 확답받는다.

Next Session Starter 블록 작성 시 위에서 확보한 REPO 값을 실제 값으로 치환해 handoff 본문에 포함한다. 예시 template의 repo slug(`greenheadHQ/nixos-config` 등) 하드코딩을 그대로 두지 않는다.
...
5. **NSS placeholder 검증 (필수)**: Next Session Starter 블록의 REPO가 다음 중 하나라도 해당하면 Step 1-C "값 확보 실패 처리" 순서로 실제 값 확보 후 치환. 치환 완료 전에는 Step 9(게시)로 진행하지 않는다.
...
- **QA 체인**: 스킬 관련 이슈의 기본 QA 체인은 `/run-da for_pr` + skill-creator 플러그인(evals/queries.json 검증, 해당 스킬에 `evals/queries.json`이 있을 때만)이다. write-handoff는 현재 `evals/queries.json`이 부재하므로 skill-creator eval 경로가 적용되지 않으며, 회귀 방지는 `/run-da for_pr` + 필요 시 `/parallel-audit`에 의존한다. `/parallel-audit`는 다중 스킬 영향·광범위 사이드이펙트·고위험 변경일 때만 조건부로 추가한다. 검증 축(frontmatter/구조/links/evals)은 유지하고 도구만 활성 경로로 교체한다.
