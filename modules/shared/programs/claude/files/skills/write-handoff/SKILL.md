...
**주의 (bare 번호 입력 시 cwd 의존)**: `$ARGUMENTS`가 `123`, `#123` 같은 bare 번호이고 cwd가 handoff 대상 repo와 다른 경우, helper는 cwd fallback(우선순위 2)으로 **잘못된 repo slug를 placeholder 검증을 통과하는 형태로 반환**할 수 있다. `gh issue view --json`은 `repository` 필드를 지원하지 않으므로 bare 번호에서 실제 issue repo를 직접 resolve할 방법이 없다. bare 번호 입력 시 LLM은 결과를 맹신하지 말고:
