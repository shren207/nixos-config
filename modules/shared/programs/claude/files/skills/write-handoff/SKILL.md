...
- **NSS 동작**:
  1. `git fetch origin "issue/{N}"` 성공이 자동 복구의 전제 (stale remote-tracking/local ref 신뢰 차단).
  2. fetch 성공 시 `origin/issue/{N}` ref가 있으면 `git switch -C "issue/{N}" --track "origin/issue/{N}"`으로 local branch를 origin tip에 강제 동기화 (stale local commit 재사용 방지).
  3. origin에는 없지만 local에 `issue/{N}`이 있으면 (fetch 성공 전제상 흔치 않은 케이스) `git switch "issue/{N}"` fallback.
  4. fetch 실패 또는 ref 부재 시 서브쉘이 `exit 1`로 종료되면서 `git log --all --grep='#{N}'` 힌트를 출력하여 실행자가 수동으로 작업 branch를 결정하도록 한다 (fail-closed).
