...
3. 확신이 없으면 런타임 도구 매핑 표의 질문 도구로 **이슈 URL**을 사용자에게 명시적으로 확답받고, helper를 URL 인자로 재실행한다. **repo slug만 받는 경로는 사용하지 않는다** — Step 1-A가 이미 bare 번호로 cwd repo 이슈를 읽었고 Step 9가 bare 번호로 코멘트를 게시하므로, REPO만 교정하면 wrong repo 이슈에 코멘트가 게시되는 fail-open 경로가 열린다. 이슈 URL을 받아 helper 재실행하면 이슈 식별자와 REPO 양쪽이 함께 교정된다.
...
처리 절차:
1. helper를 재실행한다. bare 번호 + cwd 불일치의 경우 사용자에게 **이슈 URL**을 확답받은 뒤 helper를 URL 인자로 재실행한다 (이슈 식별자와 REPO 양쪽 동시 교정). **repo slug만 받는 경로는 금지** — Step 1-A가 이미 bare 번호로 cwd repo 이슈를 읽었고 Step 9가 bare 번호로 코멘트를 게시하므로, REPO만 교정하면 wrong repo 이슈에 코멘트가 게시되는 fail-open 경로가 열린다.
2. 여전히 실패하면 런타임 도구 매핑 표의 질문 도구로 사용자에게 이슈 URL을 재차 확답받고 helper URL 인자로 재실행한다.
