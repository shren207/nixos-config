...
우선순위 (helper 내부):
1. `$ARGUMENTS`의 이슈 URL 파싱 (cwd 무관)
2. cwd repo의 `gh repo view --json nameWithOwner`
3. 둘 다 실패하면 빈 줄 → 아래 **1-D. 값 확보 실패 처리**

**주의 (bare 번호 입력 시 cwd 의존)**: `$ARGUMENTS`가 `123`, `#123` 같은 bare 번호이고 cwd가 handoff 대상 repo와 다른 경우, helper는 cwd fallback으로 **잘못된 repo slug를 placeholder 검증을 통과하는 형태로 반환**할 수 있다. bare 번호를 받으면 LLM은 결과를 맹신하지 말고:
1. `gh issue view <number> --json repository -q '.repository.nameWithOwner'`로 이슈 실제 repo를 재확인하거나,
2. 입력으로 이슈 URL을 다시 요구하여 helper 재실행.
cwd repo slug와 이슈 실제 repo가 다르면 이슈 URL 기반 값으로 치환한다.
...
**Shell-안전성 가드 (필수)**: NSS 블록은 `REPO='...'` / `BRANCH='...'` 처럼 single-quoted literal로 값을 삽입하여 `$(...)`, 백틱, `$var` 해석을 차단한다. 확보한 값에 **`'`(single quote) 또는 `\`(backslash)** 가 포함되면 치환 즉시 게시를 중단하고 질문 도구로 사용자에게 escape 전략을 확답받는다 (Git은 branch 이름에 이런 문자를 허용하지만 NSS의 복붙 실행 컨텍스트에서는 안전하지 않다).
...
6. **Shell-안전성 재검증 (필수)**: NSS 블록의 `REPO=` / `BRANCH=` 할당이 **single-quoted literal(`'...'`)** 형태인지 확인하고, 값에 `'` 또는 `\`가 포함되어 있으면 Step 9를 중단한다 (Step 1-D Shell-안전성 가드 참조).
