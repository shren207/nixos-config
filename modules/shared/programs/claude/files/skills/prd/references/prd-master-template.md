...
- Validation surface: [가용 검증 도구/gap — 상세 가이드는 `~/.claude/skills/prd/references/validation-paths.md` 참조]
...
- 검증 방법은 risk와 가용 도구에 맞춰 선택한다. 모든 phase에 동일 tool을 기본값으로 사용하지 않는다 (`~/.claude/skills/prd/references/validation-paths.md` 참조).
...
`~/.claude/skills/prd/references/multi-pass-review.md`의 체크리스트를 여기 인라인으로 복제하거나, 해당 경로를 참조 문자열로만 둔다.
