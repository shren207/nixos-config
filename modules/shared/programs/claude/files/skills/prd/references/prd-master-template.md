...
- Validation surface: [가용 검증 도구/gap — 상세 가이드는 ../references/validation-paths.md 참조]
...
- 검증 방법은 risk와 가용 도구에 맞춰 선택한다. 모든 phase에 동일 tool을 기본값으로 사용하지 않는다 ([`validation-paths.md`](./validation-paths.md) 참조).
...
[`multi-pass-review.md`](./multi-pass-review.md)의 체크리스트를 여기 인라인으로 복제하거나, 해당 파일을 참조 링크로만 둔다.
