...
SessionEnd 훅에 의해 세션 종료 시 자동으로 아카이빙되며, 수동으로도 사용 가능하다.
...
```bash
~/.claude/scripts/claude-archive.sh --session <session-id>
```
...
├── <uuid>.jsonl        # 원본 대화 데이터 (authoritative source)
├── <uuid>.md           # Markdown 변환본 (lossy 요약)
├── subagents/          # 서브에이전트 대화
...
- **JSONL**: 원본 데이터. 완전한 대화 기록.
- **Markdown**: lossy 요약본. thinking 블록 제외, tool input/result truncate, system 미포함.
- **meta.json**: `message_count`는 raw JSONL의 user+assistant 엔트리 합계 (MD 섹션 수와 다를 수 있음).
