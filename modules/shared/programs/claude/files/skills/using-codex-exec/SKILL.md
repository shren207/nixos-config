...
| `--json` | JSONL 이벤트 출력 |
| `-o, --output-last-message <FILE>` | 마지막 메시지 파일 저장 (**review에서 upstream bug openai/codex#12502로 빈 파일 생성**) |
...
`-o`는 review --help에 표시되지만 upstream bug(openai/codex#12502)로 빈 파일을 생성한다.
...
| 금지 패턴 | 발생 에러 | 올바른 대안 |
|-----------|----------|------------|
| review에서 `-o` 결과 저장 의존 | 빈 파일 생성 (upstream bug openai/codex#12502) | stdout 리다이렉트 `> file 2>&1` |
