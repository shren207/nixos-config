...
| 포함 | 제외 |
|------|------|
| `codex exec` 비대화형 실행 | direct Codex 세션의 기본 subagent fan-out (`run-da`, `parallel-audit`, `plan-with-questions`) |
...
6. **Bash tool sandbox에서 `&` + `$!` 미작동**: Claude Code의 Bash tool에서 background process PID 캡처(`$!`)가 리터럴 문자열로 반환됨. shell-level 병렬 대신 **여러 병렬 Bash tool 호출** (예: fallback 경로의 `run-da`/`parallel-audit`) + `"$(cat file)"` 인라인 인자를 사용한다. 이 제약은 direct Codex native subagent path에는 적용되지 않는다. 상세: [known-issues.md](references/known-issues.md) §11
