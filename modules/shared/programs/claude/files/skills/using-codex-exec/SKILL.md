...
| `--disable plugins` | subprocess 감사/리뷰에서 plugin 스킬·도구 surface 비활성화 |
...
| `--json` | JSONL 이벤트 출력 |
| `-o, --output-last-message <FILE>` | 마지막 메시지 파일 저장 (**review에서는 빈 파일 생성 가능**) |
...
표 안의 모든 `codex exec` 호출에는 `env CODEX_PROGRAMMATIC=1`을 codex 프로세스에 적용한다. Codex 0.124+ user-level hooks의 early-exit 신호다.
...
`-o`는 review --help에 표시되지만 빈 파일을 생성할 수 있다.
...
8. **Skill context budget warning**: `Warning: Exceeded skills context budget`가 subprocess 시작 시 보이면 plugin 또는 user-global 스킬 설명까지 모델 입력에 들어간 것이다. 감사/리뷰 subprocess는 scratch cwd, scratch `CODEX_HOME`(기존 `auth.json`만 복사), `--ignore-user-config`, `--disable plugins`, `--ephemeral`을 함께 사용해 경고와 토큰 낭비를 줄인다. plugin 도구 자체가 필요한 작업에서는 끄지 않는다.
...
| 금지 패턴 | 발생 에러 | 올바른 대안 |
|-----------|----------|------------|
| review에서 `-o` 결과 저장 의존 | 빈 파일 생성 가능 | stdout 리다이렉트 `> file 2>&1` |
