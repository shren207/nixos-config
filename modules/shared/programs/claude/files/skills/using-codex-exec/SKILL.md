...
6. **Bash tool sandbox에서 `&` + `$!` 미작동**: Claude Code의 Bash tool에서 background process PID 캡처(`$!`)가 리터럴 문자열로 반환됨. shell-level 병렬 대신 **여러 병렬 Bash tool 호출** (예: codex exec 경로의 `run-da`/`parallel-audit`) + `"$(cat file)"` 인라인 인자를 사용한다. 이 제약은 Codex 세션의 native subagent 경로에는 적용되지 않는다. 상세: [known-issues.md](references/known-issues.md) §11
7. **`< /dev/null`로 stdin 닫기 (background hang 방지)**: Claude Code Bash tool에서 병렬 호출 시 codex exec가 background로 자동 전환되면, stdin이 닫히지 않아 `Reading additional input from stdin...`에서 hang이 발생한다. 병렬/`run_in_background` 환경에서 필수. 단독 foreground에서도 무해하므로 추가 권장: `codex exec --full-auto --ephemeral -o "$DIR/result.md" "$(cat prompt.md)" < /dev/null 2>"$DIR/stderr.log"`. 상세: [known-issues.md](references/known-issues.md) §13
