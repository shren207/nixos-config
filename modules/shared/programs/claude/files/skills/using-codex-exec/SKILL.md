...
7. **`< /dev/null`로 stdin 닫기 (background hang 방지)**: Claude Code Bash tool에서 병렬 호출 시 codex exec가 background로 자동 전환되면, stdin이 닫히지 않아 `Reading additional input from stdin...`에서 hang이 발생한다. 모든 codex exec 호출에 `< /dev/null`을 추가하여 방지한다: `codex exec --full-auto --ephemeral -o "$DIR/result.md" "$(cat prompt.md)" < /dev/null 2>"$DIR/stderr.log"`. 상세: [known-issues.md](references/known-issues.md) §13
