...
| `codex exec review` 코드 리뷰 | Codex 설정 파일 전체 관리 |
...
6. **Bash tool sandbox에서 `&` + `$!` 미작동**: Claude Code의 Bash tool에서 background process PID 캡처(`$!`)가 리터럴 문자열로 반환됨. shell-level 병렬 대신 **여러 병렬 Bash tool 호출** (예: `run-da` 기본 4, `parallel-audit` 기본 6, LLM 블로킹 방지 시 `run_in_background: true`) + `"$(cat file)"` 인라인 인자를 사용한다. 상세: [known-issues.md](references/known-issues.md) §11
