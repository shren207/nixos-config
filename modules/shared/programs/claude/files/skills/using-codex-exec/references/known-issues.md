...
- **review 서브커맨드에서 `-o` 사용 시**: 빈 파일이 생성될 수 있다. §2 참조.
...
(programmatic codex 호출은 모두 `env CODEX_PROGRAMMATIC=1`을 codex 프로세스에 적용한다.)
...
**배경**: §13은 "모든 codex exec 호출에 `< /dev/null`을 추가"하여 stdin hang을 해결했다. 그러나 `< /dev/null`이 있어도 비결정적으로 hang이 발생하는 사례가 있다. stdin pipe(`cat file | codex exec ... -`)는 pipe EOF 메커니즘으로 stdin을 닫아 동일 문제를 더 확실히 해결한다.

**근본 원인 정정**: "`"$(cat file)"`에서 diff의 shell 메타문자가 재해석된다"는 가설은 PoC로 반증됨. `"$(cat file)"` 패턴에서 파일 내용의 `$()`, backtick, `<`, `>` 등은 shell이 재해석하지 않는다 (따옴표 안의 명령 치환 결과는 리터럴로 전달됨). 실제 원인은 §13과 동일한 "background 전환 시 stdin 미닫힘"이다.
...
**실증**: 다수 codex exec를 `cat file | env CODEX_PROGRAMMATIC=1 codex exec ... - (run_in_background: true)`로 병렬 실행하면 `-o` 결과 파일이 정상 생성된다.
