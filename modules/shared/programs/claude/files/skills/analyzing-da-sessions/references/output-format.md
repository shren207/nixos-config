...
```markdown
## M-5: selective consistency stability_status 분포 (n=<selective trigger findings>)

| stability_status | 카운트 | 비율 |
|------------------|--------|------|
| stable | 42 | 80.8% |
| split | 7 | 13.5% |
| fragmented | 2 | 3.8% |
...
source: fleiss-kappa.py aggregate envelope (1차) / round summary `selective:` 라인 fallback / 둘 다 부재 시 unavailable.
```
...
- partial_failure_count: 3건
- verdict_source_distribution (high+medium만): 위 M-2 source 분포 인용
```
...
```json
{
  ...
  "metrics": {
    ...
    "M-5": {
      "source": "fleiss-kappa.py" ,
      "n": 52,
      "distribution": {"stable": 42, "split": 7, "fragmented": 2, "unknown": 1, "N/A": 0},
...
`warnings`에는 SSH 실패, fleiss-kappa.py 부재, manifest.json 누락 등 partial result 사유를 기록한다.
