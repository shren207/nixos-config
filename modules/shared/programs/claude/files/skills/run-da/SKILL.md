...
5. findings 1건 이상 → Arbiter 실행:
   ...
   - 결과를 수집하여 사용자에게 전건 보고한다 (vote-shape가 있으면 함께 보고):
     ...
     - CONFIRMED_ISSUE + HIGH/MEDIUM/LOW + (N/A 또는 stable): 자동으로 계획에 반영한다.
     - NOT_AN_ISSUE + (N/A 또는 stable): 보고만 (반영 불필요).
     - NEEDS_MORE_INFO 또는 split: AskUserQuestion으로 사용자 판단을 요청한다 (vote-shape와 minority verdict도 함께 보고).
     - fragmented: **BLOCKED** — AskUser 지원 런타임에서는 판단 요청, 미지원 런타임에서는 자동 승격 금지(중단 보고).
...
5. findings 1건 이상 → Arbiter 실행:
   ...
   - 결과를 수집하여 사용자에게 전건 보고한다 (vote-shape가 있으면 함께 보고):
     - CONFIRMED_ISSUE + CRITICAL + (N/A 또는 stable): **진행 차단** (현재 라운드 중단 → 즉시 수정 → 수정 확인 후 다음 라운드 진행).
     - CONFIRMED_ISSUE + HIGH/MEDIUM/LOW + (N/A 또는 stable): 자동으로 코드에 반영하고 커밋한다.
     - NOT_AN_ISSUE + (N/A 또는 stable): 보고만 (반영 불필요).
     - NEEDS_MORE_INFO 또는 split: AskUserQuestion으로 사용자 판단을 요청한다 (vote-shape와 minority verdict도 함께 보고).
     - fragmented: **BLOCKED** — AskUser 지원 런타임에서는 판단 요청, 미지원 런타임에서는 자동 승격 금지(중단 보고).
6. 반영 후 동일 선택 review unit을 **새 reviewer 실행 단위**로 재실행한다.
