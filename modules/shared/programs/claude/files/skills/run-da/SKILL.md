...
> **Claude Code 세션 전용 주의** — Claude Code의 Bash tool은 호출마다 독립 shell에서 실행되어 환경변수가 다음 호출로 전달되지 않는다 (`$DA_DIR` 유실). `mktemp -d` 결과를 다음 호출에서 쓰려면 (1) 단일 Bash 호출 안에 체이닝하거나 (2) `echo`로 경로를 출력해 다음 호출에서 리터럴로 재사용한다. Codex 세션 · headless 세션은 이 제약이 없으므로 이 주의는 Claude Code에 국한한다.
...
- **셸 호출 간 변수 유지** (Claude Code 세션 전용): 위 "런타임 도구 매핑" 박스의 Claude Code 전용 주의 참조. Codex 세션 · headless 세션은 이 제약 없음. 상세 패턴은 [`arbiter-scaling.md`](references/arbiter-scaling.md)의 "Bash tool 변수 유실 방지" 참조.
...
| standard review profile | Codex 세션 reviewer/auditor/Intensity subagent는 `model="gpt-5.5"`와 `reasoning_effort="medium"`로 실행한다. Claude Code에서는 `model: "sonnet"` | DA/Intensity spawn 기본값 |
...
1. **codex exec subprocess fallback** (1순위): `exec_command`로 `codex exec --full-auto --ephemeral -c model_reasoning_effort="medium" - 2>stderr.log`를 호출해 각 Intensity/reviewer/Arbiter unit을 **독립 프로세스**로 순차 실행한다. fresh subagent의 독립 판정 경계가 codex exec 프로세스 경계로 보존된다. 사용자 질문 없이 자동 전환.
2. **BLOCKED 중단** (2순위): `codex` binary 미설치/접근 불가 시 현재 DA 라운드 중단 → 라운드 요약에 `"delegation unavailable + codex exec unavailable → BLOCKED"` 명시 → 사용자에게 plain-text 보고.

라운드 요약에는 경로를 다음 예시처럼 명시한다:

```text
Round N 요약: fan-out 경로 = codex exec subprocess serial fallback (reason: delegation unavailable)
```
...
3. **codex exec 경로** (Claude Code 세션 · headless 세션):
   - Claude Code 세션은 상단 "런타임 도구 매핑"의 Claude Code 전용 주의(셸 호출 간 변수 유지)를 따른다. Codex 세션 · headless 세션은 해당 없음.
...
2. 선택된 reviewer bundle 또는 explicit exhaustive override의 세부 도메인별 DA 에이전트를 **병렬 실행**한다.
   ...
   - **codex exec 경로** (Claude Code 세션 · headless 세션):
     ...
     - 세션별 임시 디렉토리를 생성한다: `DA_DIR=$(mktemp -d /tmp/da-${_DA_SID}-plan-XXXXXX)`. Claude Code 세션은 상단 "런타임 도구 매핑"의 셸 호출 간 변수 유지 주의를 따른다 (경로를 `echo`로 출력하여 이후 호출에서 리터럴로 재사용).
     - 선택된 review unit별 프롬프트 파일을 생성한다: `$DA_DIR/{unit}.md`
     - 선택된 review unit 수만큼 `cat "$DA_DIR/{unit}.md" | codex exec --full-auto --ephemeral -c model_reasoning_effort="medium" -o "$DA_DIR/{unit}-result.md" -`를 병렬 실행으로 띄운다 (런타임별 병렬 실행 매커니즘은 "런타임 도구 매핑" 표 참조).
     - 병렬 실행 완료 알림을 수신하면 sleep/poll 없이 바로 결과를 수집한다.
...
3. 모든 reviewer 결과를 수신한 후 종합 리포트를 작성한다.
   ...
   - codex exec 경로: 4개 병렬 실행 완료 후, 각 `$DA_DIR/{bundle}-result.md`를 파일 읽기 도구로 명시적으로 읽어 수집한다. 결과 파일이 없거나 빈 경우, 또는 exit code가 0이 아니면 실패로 판정한다.
...
2. 선택된 reviewer bundle 또는 explicit exhaustive override의 세부 도메인별 DA 에이전트를 **병렬 실행**한다.
   - **Codex 세션 경로**:
     ...
     - `fresh` modifier와 selective propagation 규칙은 동일하게 적용한다.
   - **codex exec 경로** (Claude Code 세션 · headless 세션):
     - 실행 전 [`../using-codex-exec/SKILL.md`](../using-codex-exec/SKILL.md)의 패턴 4 (exec 우회)를 참조한다.
     - 라운드별 임시 디렉토리를 생성한다: `DA_DIR=$(mktemp -d /tmp/da-${_DA_SID}-pr-XXXXXX)`. Claude Code 세션은 상단 "런타임 도구 매핑"의 셸 호출 간 변수 유지 주의를 따른다 (경로를 `echo`로 출력하여 이후 호출에서 리터럴로 재사용).
     - 선택된 review unit별 프롬프트 파일을 생성한다: `$DA_DIR/{unit}.md`
     - 선택된 review unit 수만큼 `cat "$DA_DIR/{unit}.md" | codex exec --full-auto --ephemeral -c model_reasoning_effort="medium" -o "$DA_DIR/{unit}-result.md" -`를 병렬 실행으로 띄운다 (런타임별 병렬 실행 매커니즘은 "런타임 도구 매핑" 표 참조).
...
3. **zsh 고정 가정**: codex exec 경로의 `_DA_SID` 해시 계산, cleanup glob `*(N)` qualifier, heredoc 문법 등은 **zsh 전제**다. bash/sh 환경에서는 `*(N)`이 문법 오류가 난다. POSIX-safe 변형은 별도 follow-up (예: guardrail 스킬에서 shell 전제 lint).
