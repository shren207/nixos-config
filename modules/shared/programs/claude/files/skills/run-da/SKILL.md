...
1. **BLOCKED + 사용자 승인 대기** (기본): `spawn_agent` 거부 감지 시 현재 DA 라운드 중단, 사용자에게 "delegation 거부 감지 — codex exec subprocess fallback 승인?"을 보고한다. 승인 수단은 런타임별로 다음과 같이 취한다:
   - 질문 도구 지원 런타임 (Claude Code 세션, Codex Plan mode `request_user_input` 지원 시): 질문 도구로 즉시 승인 요청.
   - 질문 도구 미지원 런타임 (Codex 일반 세션 등): plain-text로 상황 보고 후 DA 루프 종료. 사용자가 다음 메시지에서 "fallback 진행"으로 명시 승인하거나 `run-da <mode> fallback-approved` 재실행 인자로 기동하면 fallback 단계로 재진입. 명시 승인 없이 자동 재개하지 않는다.
...
2. 선택된 reviewer bundle 또는 explicit exhaustive override의 세부 도메인별 DA 에이전트를 **병렬 실행**한다.
   ...
   - **codex exec 경로** (Claude Code 세션 · headless 세션):
     ...
     - 선택된 review unit 수만큼 `cat "$DA_DIR/{unit}.md" | codex exec --full-auto --ephemeral -c model_reasoning_effort="medium" -o "$DA_DIR/{unit}-result.md" -`를 런타임별 병렬/순차 실행으로 기동한다 — Claude Code 세션은 `Agent`/`Bash tool` + `run_in_background: true` 병렬, **headless 세션은 serial foreground** (완료 알림·`&+wait` 없음). 런타임별 매커니즘은 "런타임 도구 매핑" 표 참조.
     - Claude Code 세션: 병렬 실행 완료 알림을 수신하면 sleep/poll 없이 바로 결과를 수집한다. headless 세션: 각 subprocess 종료를 직렬로 확인한다.
     - 모든 런타임 공통: `& + wait` shell-level 병렬 금지, `cat file | codex exec ... -` stdin pipe로 프롬프트 전달. pipe EOF가 stdin을 닫으므로 `< /dev/null`은 불필요. 인라인 인자 `"$(cat file)"`는 사용하지 않는다.
...
3. 모든 reviewer 결과를 수신한 후 종합 리포트를 작성한다.
   ...
   - codex exec 경로: 4개 실행(Claude Code는 병렬, headless는 serial) 완료 후, 각 `$DA_DIR/{bundle}-result.md`를 파일 읽기 도구로 명시적으로 읽어 수집한다. 결과 파일이 없거나 빈 경우, 또는 exit code가 0이 아니면 실패로 판정한다.
...
2. 선택된 reviewer bundle 또는 explicit exhaustive override의 세부 도메인별 DA 에이전트를 **병렬 실행**한다.
   ...
   - **codex exec 경로** (Claude Code 세션 · headless 세션):
     ...
     - 라운드별 임시 디렉토리를 생성한다: `DA_DIR=$(mktemp -d /tmp/da-${_DA_SID}-pr-XXXXXX)`. 모든 런타임은 상단 "런타임 도구 매핑" 아래의 공통 주의(셸 호출 간 변수 유실)를 따른다 (경로를 `echo`로 출력하여 이후 호출에서 리터럴로 재사용).
     - 선택된 review unit별 프롬프트 파일을 생성한다: `$DA_DIR/{unit}.md`
     - 선택된 review unit 수만큼 `cat "$DA_DIR/{unit}.md" | codex exec --full-auto --ephemeral -c model_reasoning_effort="medium" -o "$DA_DIR/{unit}-result.md" -`를 런타임별로 기동한다 — Claude Code 세션은 병렬(`run_in_background: true`), **headless 세션은 serial foreground**. 매커니즘은 "런타임 도구 매핑" 표 참조.
     - 모든 런타임 공통: `cat file | codex exec ... -` stdin pipe로 프롬프트를 전달하고, `& + wait` shell-level 병렬 금지. pipe EOF가 stdin을 닫으므로 `< /dev/null`은 불필요. 인라인 인자 `"$(cat file)"`는 사용하지 않는다.
3. 모든 reviewer 결과를 수신한 후 종합 리포트를 작성한다.
   - Codex 세션 경로: `wait_agent` 결과를 집계한 뒤, 다음 round/retry 전에 completed reviewer thread를 `close_agent`로 닫는다.
   - Codex 세션 경로: `VIOLATION` 처리 규칙은 위 `Codex 세션 하드닝 계약`의 공통 처리 정의를 따른다. offending unit은 rerun 또는 `BLOCKED` 해소 전까지 `CLEAR` 계산에 포함하지 않는다.
