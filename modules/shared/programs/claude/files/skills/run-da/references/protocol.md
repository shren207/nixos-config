...
| 합리화 패턴 | 왜 틀렸는가 |
|---|---|
| "이건 단순한 수정이라 DA가 필요 없다" | 단순한 수정에서 가장 많은 사이드이펙트가 발생한다. **예외**: SKILL.md의 Review Intensity 절차를 거쳐 AskUserQuestion으로 사용자 승인을 받은 SKIP은 합리화가 아니다. 절차를 거치지 않은 생략 시도는 여전히 금지한다. |
