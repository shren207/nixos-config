...
> `{BUNDLE}` / `{SUBDOMAINS}` / `{FOCUS_QUESTION}` 등의 UPPERCASE 표기는 플레이스홀더 관용이며, 치환 값은 아래 bundle 정의 표의 Title Case 를 **대소문자 변환 없이** 그대로 사용한다. Bash tool(zsh) 의 case modification 제약은 repo 루트 `CLAUDE.md` "Bash tool 환경" 섹션 참조.
