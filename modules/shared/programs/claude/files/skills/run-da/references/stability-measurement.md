...
VERDICT_JSON 블록의 `stability_status` 필드는 아래 4가지 중 하나다.
...
3. **Judge diversity** (Verga et al. 2024 "Replacing Judges with Juries"): v1은 동일 모델 N=3로 시작. heterogeneous judge panel은 `#523` 실험 영역.
...
- **Heterogeneous jury panel**: 서로 다른 모델 계열로 N=3 구성하는 실험은 `#523` PoLL 실험으로 deferral.
- **Fleiss kappa 외 metric**: Cohen's kappa(2인 판정 고전), Krippendorff's alpha(일반) 등은 v1 범위 아님.
- **Arbiter prompt 자체 변경**: rubric 문구 최적화, Few-shot 추가는 `#521`(Portability 축) 이외 별도 트랙.
