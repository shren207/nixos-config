...
- (c) Few-shot 예시 6(PR #513 Adjacent-1)이 cross-env drift 해석을 구체화한다.
...
#### 예시 6: for_plan CONFIRMED_ISSUE — Portability guardrail (PR #513 Adjacent-1)
...
```json
{
  ...
  "portability_applies": true | false
}
```
<!-- verdict-json:end -->
```
...
- `portability_applies`: Portability 축이 non-N/A(PASS 또는 FAIL)인 경우 `true`.
