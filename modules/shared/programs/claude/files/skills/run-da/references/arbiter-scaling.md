...
(`CODEX_CI`는 codex의 `UNIFIED_EXEC_ENV`에 하드코딩되어 모든 subprocess에 강제 주입된다.)

Codex에서 run-da 실행 시 AskUserQuestion(`request_user_input`) 미지원:
...
- Codex 환경(`CODEX_CI=1` 설정): SKIP 판정 시 AskUserQuestion 불가이므로 자동 LITE 승격.
