...
- codex exec 경로(Claude Code 세션 · headless 세션)에서는 **foreground** Bash tool 호출로 `--full-auto --ephemeral -c model_reasoning_effort="medium"`를 실행한다 (단일 exec, `run_in_background` 사용 안 함).
