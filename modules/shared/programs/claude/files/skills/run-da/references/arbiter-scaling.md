...
- 매 실행마다 fresh Arbiter 또는 Intensity subagent 1개를 strong review profile(`model="gpt-5.4"`, `reasoning_effort="xhigh"`)로 사용한다.
...
1. Arbiter 또는 Intensity용 fresh subagent를 1개 strong review profile로 띄운다.
...
- `stateful violation`: tracked write, branch mutation, GitHub write, main-agent-only command 실행, host mutation처럼 상태를 바꾼 위반. 현재 라운드를 즉시 중단하고 offending thread를 닫는다.
- stateful violation은 local tracked/untracked 변경, scratch 산출물, branch/worktree 정합성 확인이 끝나기 전까지 `CLEAR` 또는 정상 실패로 간주하지 않는다.
- 비가역적 외부 side effect가 있었으면 incident로 기록하고, AskUserQuestion 가능 시 사용자에게 보고한다. AskUserQuestion 미지원 런타임에서는 자동 `CLEAR` 처리하지 않고 `BLOCKED`로 남긴다.

## 런타임 선택 규칙

- direct Codex path는 **현재 세션이 native subagent를 직접 오케스트레이션할 수 있을 때** 기본 경로다.
- fallback / explicit `codex exec` path는 Claude Code subprocess, headless automation, 명시적 CLI 사용 시에만 기본 경로다.
- `CODEX_CI=1`은 direct Codex 세션에서도 보일 수 있으므로 sole discriminator로 쓰지 않는다.
...
- direct Codex path에서는 fresh intensity subagent 1개를 strong review profile(`model="gpt-5.4"`, `reasoning_effort="xhigh"`)로 사용하고, 결과 파싱 후 completed thread를 `close_agent`로 닫는다.
- fallback / explicit `codex exec` path에서는 `--full-auto --ephemeral`로 실행한다.
- 프롬프트에서 "references/intensity-rules.md를 직접 읽어 규칙을 적용하라"고 지시하고, Intensity는 review-only/no-write role임을 함께 명시한다.
- fallback path의 프롬프트 파일은 `umask 077`로 권한 제한한다.
