...
- direct Codex path에서는 fresh intensity subagent 1개를 standard review profile로 사용하고, 결과 파싱 후 completed thread를 `close_agent`로 닫는다.
- fallback / explicit `codex exec` path에서는 `--full-auto --ephemeral -c model_reasoning_effort="high"`로 실행한다.
