...
- codex exec 경로(Claude Code 세션 · headless 세션)에서는 **foreground 실행**으로 Layer 1 supervised wrapper(`codex-exec-supervised --sandbox read-only --ignore-user-config --ephemeral -c model="gpt-5.5" -c model_reasoning_effort="medium"`, standard review profile)를 호출한다 (단일 exec이므로 병렬/background 사용 안 함). 정확한 호출 명령은 본 파일 [`intensity-procedure.md`](intensity-procedure.md)의 codex exec 경로 코드블록 참조.
