...
- `codex exec -C "$EXEC_CWD" --sandbox read-only --ignore-user-config --disable plugins --ephemeral`
...
- 감사/리뷰 subprocess에는 repo 밖 scratch cwd와 `--ignore-user-config --disable plugins`를 붙여 project/user/plugin 외부 도구 surface와 skill context budget 경고를 차단한다.
...
**공통**:
- repo 밖 scratch cwd + `--sandbox read-only` + `--ignore-user-config` + `--disable plugins`를 함께 강제한다. `-C "$EXEC_CWD"`는 cwd 기반 project config를 피하고, `--sandbox read-only`는 model-generated shell command의 파일시스템 쓰기를 막고, `--ignore-user-config`는 user `config.toml`의 MCP server/connector 로딩을 차단하고, `--disable plugins`는 installed plugin 스킬·도구 surface와 skill context budget 경고를 차단한다.
...
- `--ephemeral`로 세션 히스토리 오염 방지.
- `exec_command`를 `EXEC_CWD="$(mktemp -d /tmp/da-${_DA_SID}-exec-cwd-XXXXXX)"; cat "$DIR/prompt.md" | env CODEX_PROGRAMMATIC=1 codex exec -C "$EXEC_CWD" --sandbox read-only --ignore-user-config --disable plugins --ephemeral ... - 2>stderr.log` 형태로 stdin pipe 전달.
...
cat "$DA_DIR/{unit}.md" | env CODEX_PROGRAMMATIC=1 codex exec -C "$EXEC_CWD" --sandbox read-only --ignore-user-config --disable plugins --ephemeral \
...
```bash
EXEC_CWD="$(mktemp -d /tmp/da-${_DA_SID}-exec-cwd-XXXXXX)"
cat "$ARBITER_DIR/arbiter-prompt.md" | env CODEX_PROGRAMMATIC=1 codex exec -C "$EXEC_CWD" --sandbox read-only --ignore-user-config --disable plugins --ephemeral \
  -c approval_policy='"never"' \
...
2. **환경 격리** — first-pass Arbiter와 selective consistency N=3 모두 strong review profile(high)을 사용한다. `~/.codex/config.toml` 기본값(xhigh)과 다르므로 **반드시 `-c model_reasoning_effort="high"`를 명시한다**. **selective consistency N=3**은 외부 표면과 충돌을 줄이기 위해 다음 두 방식 중 하나를 선택한다:
   ...
   **(a) 기본 경로 + config 차단** (권장, 간단):
   ...
   - codex exec 호출에 repo 밖 scratch cwd, `--ignore-user-config`, `--disable plugins`를 추가하여 project config, 사용자 `config.toml`(MCP 서버 포함), installed plugin surface 로딩을 차단한다. `using-codex-exec/SKILL.md:113`에 기록된 대로 `--ignore-user-config`는 **config만 차단하고 auth는 유지**한다.
...
- codex exec 경로(Claude Code 세션 · headless 세션)에서는 **foreground 실행**으로 `-C "$EXEC_CWD" --sandbox read-only --ignore-user-config --disable plugins --ephemeral -c approval_policy='"never"' -c model_reasoning_effort="medium"`를 호출한다 (단일 exec이므로 병렬/background 사용 안 함).
