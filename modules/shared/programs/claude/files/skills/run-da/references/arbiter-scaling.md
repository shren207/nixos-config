...
- codex exec 경로(Claude Code 세션 · headless 세션)에서는 **foreground 실행**으로 `--full-auto --ephemeral -c model_reasoning_effort="medium"`를 호출한다 (단일 exec이므로 병렬/background 사용 안 함).
