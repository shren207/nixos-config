...
- Review summary의 `body`는 **맥락 파악 용도**로만 읽는다. 답글/resolve 대상이 아니다. summary 본문에 actionable이 있다면 해당 지적이 review thread 또는 issue comment로도 남아 있는지 확인 후 그쪽 경로로 처리한다.
...
- [ ] Review summary 본문 보관. **non-empty summary는 Step 6의 top-level follow-up 대상**이므로 답글 경로를 타며, empty인 경우에만 no-op 가드 계산에 포함한다.
