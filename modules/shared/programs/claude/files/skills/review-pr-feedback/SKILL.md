...
검증 과정에서 다음 상황이 발생하면 추가 분석을 수행한다:

- **로컬 재현 필요**: 빌드 에러, 런타임 문제 등 실제 재현이 필요한 피드백은
  로컬에서 직접 테스트한다.
- **기술 자문 필요**: 복잡한 아키텍처 결정이나 Nix 표현식 관련 심층 분석이 필요하면
  에이전트를 활용하여 기술 자문을 구한다.
- **skills 관련 피드백**: 스킬 파일에 대한 피드백은 skill-reviewer 역할의
  에이전트를 활용한다.
- **외부 레퍼런스 필요**: 특정 라이브러리나 도구의 동작 확인이 필요하면
  에이전트를 병렬 실행하여 조사한다.
