...
- Claude Code 세션 · headless 세션에서는 bundle마다 `cat "$DA_DIR/{unit}.md" | env CODEX_PROGRAMMATIC=1 codex-exec-supervised --sandbox read-only --ignore-user-config --ephemeral -c model_reasoning_effort="medium" -o "$DA_DIR/{unit}-result.md" -` subprocess 1개를 사용한다. **`CODEX_PROGRAMMATIC=1` env assignment는 codex 프로세스에 적용되어야 한다 (회피: `CODEX_PROGRAMMATIC=1 cat ...`은 cat에만 적용 — issue #585): Codex 0.124+ user-level hooks의 early-exit 신호.**
