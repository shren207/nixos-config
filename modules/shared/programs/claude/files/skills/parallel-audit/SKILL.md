...
5. **Baseline 비교 (aggregate)** (Step 2a 출력 `$BASELINE_FILE`과 비교):
   ...
   - **delta 비교** (status-only payload 대상): 무결성 검증 통과 시 현재 `git status --porcelain=v1`을 `tail -n +2 "$BASELINE_FILE"` (sentinel header 제거한 status-only payload)과 비교한다. sentinel header를 그대로 둔 채 현재 status와 비교하면 항상 불일치로 오판되므로 반드시 `tail -n +2`로 header를 제거한 뒤 비교한다. 차이 여부 × self-report 조합에 따라 아래 두 경로를 **분리**하여 적용한다:
