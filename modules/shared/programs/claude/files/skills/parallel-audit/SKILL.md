...
5. **Baseline 비교 (aggregate)** (Step 2a 출력 `$BASELINE_FILE`과 비교):
   - **사전 무결성 검증**: `$BASELINE_FILE`이 존재하지 않거나, 비어 있거나 (`git status`가 비어 있었던 초기 상태 제외), Step 2a 직후 기록한 내용과 일치하지 않으면(메인 에이전트가 컨텍스트에 baseline content를 보존하여 비교) 즉시 `STATEFUL VIOLATION (baseline tampered or missing)`로 fail-closed BLOCKED 처리한다.
