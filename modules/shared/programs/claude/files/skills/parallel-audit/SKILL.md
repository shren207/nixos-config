...
**"나는 어떤 세션에서 실행되고 있는가?"** 로 경로를 선택한다. 런타임별 도구 binding은 [run-da의 "런타임 도구 매핑" 표](../run-da/SKILL.md#런타임-도구-매핑)가 단일 소스다. 공통 subprocess 위생/제약(세션 네임스페이스, stdin pipe, 환경변수 유실)은 [`../using-codex-exec/SKILL.md`](../using-codex-exec/SKILL.md)와 run-da의 "codex exec 경로 위생 규칙"을 따르며, Codex delegation-denied fallback 실행 계약은 [run-da references/arbiter-scaling.md](../run-da/references/arbiter-scaling.md)의 "Codex delegation-denied fallback" 섹션만 참조한다. **auditor 기본 fan-out 명령은 아래 Step 3b가 정의한다.**
...
1. **공통 요약**: diffstat, 변경 파일 목록, 루트 수준 의도 요약.
2. **bundle-scoped context**: 담당 bundle과 직접 관련된 파일 목록, diff slice, 관련 모듈 구조. 전체 diff를 기본으로 복제하지 않는다.
3. **담당 조사 bundle**: 위 테이블에서 배정된 bundle과 조사 대상.
4. **출력 형식 지시**: 아래 "결과 형식"에 따라 반환.

bundle이 전체 diff 없이는 판정할 수 없으면 auditor는 finding을 만들기 전에 `NEEDS_CONTEXT: <필요한 파일/범위>`를 반환한다. 메인 에이전트는 해당 bundle만 full diff 또는 추가 파일로 재실행한다.

에이전트 지시 원칙 (run-da Auditor 계약과 동일, `run-da/SKILL.md`의 "역할별 경계" 표 `Auditor (parallel-audit)` 행 계승):
...
- Claude Code 세션 · headless 세션에서는 bundle마다 scratch cwd와 scratch Codex home(auth.json만 복사)를 만들고 `cat "$DA_DIR/{unit}.md" | env CODEX_HOME="$EXEC_CODEX_HOME" CODEX_PROGRAMMATIC=1 codex exec -C "$EXEC_CWD" --skip-git-repo-check --sandbox read-only --ignore-user-config --disable plugins --ephemeral -c approval_policy='"never"' -c model='"gpt-5.5"' -c model_reasoning_effort='"medium"' -o "$DA_DIR/{unit}-result.md" -` subprocess 1개를 사용한다. **`CODEX_PROGRAMMATIC=1` env assignment는 codex 프로세스에 적용되어야 한다 (회피: `CODEX_PROGRAMMATIC=1 cat ...`은 cat에만 적용). Codex 0.124+ user-level hooks의 early-exit 신호다.**
...
- bundle별 병렬 실행을 수행한다. 실행 binding 상세(Claude Code 고유 fallback lifecycle, 완료 알림 수신, thread 관리)는 [run-da SKILL.md](../run-da/SKILL.md)의 Claude Code fallback 관련 섹션을 참조한다.
...
| 2 | Performance + Dependencies | BUG | modules/foo.nix:<line> — O(n^2) 루프 발견 |
...
2. **native/Agent fallback의 구조적 read-only 부재**: Codex 세션 native subagent와 Claude Code Agent fallback은 per-child read-only sandbox를 구조적으로 강제하지 못한다. Claude Code 세션과 headless 세션의 기본 `codex exec` 경로는 scratch cwd + scratch `CODEX_HOME` + `--sandbox read-only --ignore-user-config --disable plugins`로 실행해 이 한계를 줄인다. native/Agent fallback의 "읽기 전용" 경계는 **정책 + 프롬프트 계약 + self-report + sandbox 비의존 baseline 점검**의 조합으로 운영한다.
