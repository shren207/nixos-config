...
- Claude Code 세션 · headless 세션에서는 bundle마다 `cat "$DA_DIR/{unit}.md" | env CODEX_PROGRAMMATIC=1 codex exec --disable plugins --full-auto --ephemeral -c model_reasoning_effort="medium" -o "$DA_DIR/{unit}-result.md" -` subprocess 1개를 사용한다. **`CODEX_PROGRAMMATIC=1` env assignment는 codex 프로세스에 적용되어야 한다 (회피: `CODEX_PROGRAMMATIC=1 cat ...`은 cat에만 적용 — openai/codex#585): Codex 0.124+ user-level hooks의 early-exit 신호.**
...
2. **auditor 기본 실행 경로의 workspace-write 허용**: Claude Code 세션과 headless 세션의 기본 경로는 `codex exec --disable plugins --full-auto` (workspace-write)다. 이는 run-da canonical contract와 일관된 선택이며, read-only sandbox 구조적 enforcement는 없다. auditor의 "읽기 전용" 경계는 **정책 + 프롬프트 계약 + self-report + sandbox 비의존 baseline 점검**의 조합으로 운영한다. `--disable plugins`는 plugin 스킬/도구 surface와 skill context budget 경고만 줄이며 filesystem sandbox를 read-only로 바꾸지는 않는다. `--sandbox read-only` 경로는 run-da의 "Delegation fallback"(사용자 승인 필수)에만 적용된다.
