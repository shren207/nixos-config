...
| `--resume <SESSION_ID>` | 이전 세션 이어서 실행 | `-p`에서 세션 체이닝 가능 |
...
| `--allowed-tools <TOOLS>` | 허용할 도구 목록 (쉼표 구분) | ⚠️ 인라인 프롬프트가 도구 이름으로 파싱됨 → stdin 필수 |
...
| `--permission-prompt-tool <TOOL>` | MCP 도구에 퍼미션 처리 위임 | CI/CD 시스템에서 자체 퍼미션 UI 연동용 |
...
| `allowEdits` | 편집만 허용 | ✅ | ✅ | 파일 편집만 자동 승인 |
