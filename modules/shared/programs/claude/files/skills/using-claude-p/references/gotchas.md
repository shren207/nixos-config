...
### `--allowedTools` 뒤에 인라인 프롬프트가 도구 이름으로 파싱됨
...
### 인라인 인수만 쓸 때 stdin이 tty면 EOF 대기하며 hang
...
### 커스텀 환경변수는 `VAR=val claude -p` 형태로 명시적 전달 필요
...
### `-p`에서 `--verbose`/`--debug`는 stderr에 아무것도 출력 안 함
...
### `--max-budget-usd` 초과도 exit code 0
...
### `--max-turns`는 `--help`에 표시되지 않는 숨겨진 플래그
...
### 권한 없이 도구 사용 시 조용히 거부, exit code 0
...
### `--dangerously-skip-permissions`는 hooks의 block을 무시하지 않음

Permission prompt만 건너뛰고, hooks 자체는 호출된다. 단, `bypassPermissions` 모드에서는 hooks의 결정이 passthrough로 무시된다. ⚠️ `--disable-hooks` 플래그는 v2.1.76에 존재하지 않음.

### `bypassPermissions` 모드에서 hooks는 호출되지만 결정이 passthrough로 무시됨

hooks 자체는 실행되지만, hooks의 allow/deny/block 결정이 결과에 반영되지 않는다. 이는 hooks block 처리 항목과 일관됨: `--dangerously-skip-permissions`(= `bypassPermissions`)는 permission prompt를 건너뛰고, hooks 결정도 무시한다.

### `default` 모드에서 hooks의 allow/deny/block 결정은 정상 반영됨

비대화형 모드에서도 `default` 퍼미션 모드를 사용하면 hooks의 결정이 존중된다.

### `--permission-prompt-tool`로 MCP 도구에 퍼미션 처리 위임 가능
...
### `--tools ""`로 빌트인 비활성화해도 MCP 도구는 남아있음
...
### `--disable-slash-commands`로 스킬 비활성화 시 "Unknown skill"
...
### `allowedTools` 패턴 공백 의미 차이

`allowedTools` 공백 항목 참조.

---

## hooks

### Notification hook은 `-p`에서 트리거되지 않음
...
### CLAUDE.md, skills, plugins, hooks, MCP 서버 전부 로드됨
...
### `--resume SESSION_ID`가 `-p`와 함께 동작 (세션 체이닝)
...
### `--append-system-prompt`는 override가 아닌 append
...
### SSH non-login shell에서 aliases 미로드
...
### 공식 명칭 변경: "headless mode" → "Agent SDK"
...
### `CLAUDE_CODE_MAX_RETRIES` 환경변수로 API 재시도 횟수 제어
