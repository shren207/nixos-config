...
description: |
  Manage status bar icons (Jira, Slack, Figma, Memo, Memory) for Claude Code sessions.
...
4. **아이콘 순서 변경 불가**: 순서는 `statusline.sh`에 하드코딩 (Jira → Slack → Figma → Plan → Memo → Memory)
