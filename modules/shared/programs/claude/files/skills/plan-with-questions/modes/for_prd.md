...
- **각 phase 종료 시** (Implementation 진행 중): `/prd` 정본 phase template의 **Phase-End 10-pass**를 그대로 수행하고, 추가로 `/review-implementation` 6-classification을 보조 layer로 적용한다 (둘 다 수행, 대체 아님). 두 review의 owner는 모두 `/prd` (정본) — plan-with-questions는 cherry-pick 호출만 위임.
  - Phase-End 10-pass: `/prd/references/phase-template.md`의 phase-end review 그대로.
  - 6-classification: requirement → status (`satisfied | partial | missing | conflicting | overbuilt | deferred`) → code evidence → gap → action.
  - `overbuilt` 발견 시 PRD/phase 파일에 Decision Log DL 추가 + 다음 phase 시작 전 메인 에이전트가 직접 제거 (auto-fix 미사용 — NG-2).
- **Post-Impl 5번 Final review**: `prd/references/multi-pass-review.md` Final 10-pass + `/review-implementation` 9-pass review-only를 메인 에이전트가 직접 통합 호출 (fan-out 금지). 결과는 `/prd`의 Final closeout 갱신에 반영.
