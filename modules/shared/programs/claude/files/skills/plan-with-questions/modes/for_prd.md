...
PRD가 작성된 후 구현 단계는 [`../references/post-implementation.md`](../references/post-implementation.md)를 따르되, split mode의 pending phase는 phase-scoped `PHASE-*` chain으로 실행하고 모든 split PRD의 final review·follow-up commit·PR write는 final closeout gates로 승인받는다:
