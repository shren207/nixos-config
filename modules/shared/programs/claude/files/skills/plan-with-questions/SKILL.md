...
| 핵심 도구 | AskUserQuestion, EnterPlanMode, ExitPlanMode | AskUserQuestion, codex exec / Agent / native subagent(런타임 분기), /create-issue |
...
- "추후 결정", "별도 검토 필요", "적절히 처리", "필요에 따라" 등 미결정 표현을 금지한다.
  ...
  단, `[UNVERIFIED]` 라벨처럼 검증 상태를 명시하는 표기는 허용한다 (상세는 `write-handoff/references/llm-friendly-checklist.md`의 라벨 체계 참조).
