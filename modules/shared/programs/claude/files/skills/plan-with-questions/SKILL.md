...
**사용자에게 질문할 때는 런타임 도구 매핑 표의 질문 도구를 사용한다.**
...
- **Claude Code 세션**: `EnterPlanMode` 진입 시점부터 write 작업이 제한되므로 검증은 반드시 진입 전에 완료돼야 한다.
- **Codex 세션**: `update_plan`은 write 제한을 강제하지 않지만, Step 7 이후는 "검증 단계 종료 + 계획 작성 단계"로 규약되므로 동일하게 Step 1-6에서 검증을 완료한다.
...
계획이 완성되면 런타임 도구 매핑의 승인 요청 도구로 사용자에게 계획 승인을 요청한다.

- **Claude Code 세션**: `ExitPlanMode`가 계획 파일 내용을 사용자에게 제시하여 승인을 받는다.
- **Codex 세션**: 계획 파일 경로와 요약을 plain-text로 제시하고 사용자 confirm을 대기한다. `update_plan`은 상태 표시 전용이며 승인 기능이 없다.

사용자가 수정을 요청하면 계획 파일을 편집한 뒤 승인 요청 도구를 다시 호출한다 (Claude: `EnterPlanMode`로 재진입 → `ExitPlanMode`, Codex: 파일 편집 후 plain-text 재제시).

계획이 승인되면 (사용자가 수정 요청을 하지 않으면) 추가 확인 없이 즉시 아래 **Post-Implementation 1번**부터 진행한다. 1~7번 절차는 변경 구현·커밋·`/run-da`·`/parallel-audit`·10-pass review·`/create-pr`까지 포함하며, plan 본문의 Post-Implementation 섹션에 명시되므로 계획 승인은 이 자동 진행 범위(tracked write·commit·GitHub PR write 포함)에 대한 사용자 동의로 간주된다.
...
- `/run-da`의 `BLOCKED`, `NEEDS_MORE_INFO`, `stability_status=split`/`fragmented`, `partial_failure`, low-confidence fail-closed 승격, delegation fallback 승인 대기 등은 [`../run-da/SKILL.md`](../run-da/SKILL.md)와 [`../run-da/references/protocol.md`](../run-da/references/protocol.md)의 상태 전이표를 따른다.
...
- **질문 도구 의무**: 사용자에게 질문할 때는 런타임 도구 매핑 표의 질문 도구를 사용한다. 이 규칙은 예외 없이 적용된다.
