...
- [`./prd/phase-template.md`](./prd/phase-template.md) 의 Phase-End 10-pass 를 그대로 수행한다. 10-pass 의 10개 축은 intent, correctness, simplicity, code quality, cleanup, security, performance, validation, future-phase, PRD sync 다.
- 추가로 [`./review-impl/requirement-status.md`](./review-impl/requirement-status.md) 의 6-classification 을 보조 layer 로 적용한다. 6분류는 satisfied, partial, missing, conflicting, overbuilt, deferred 다.

**Final review (PRD 10-pass + review-impl overlay)**:

- 모든 phase 완료 후 Post-Impl 의 Step 5 에서 [`./prd/multi-pass-review.md`](./prd/multi-pass-review.md) 의 PRD 10-pass 를 SSOT 체크리스트로 수행한다. input 은 PRD master + phase 파일 + 구현 코드다.
- 결과 위에 [`./review-impl/implementation-review.md`](./review-impl/implementation-review.md) overlay 를 얹는다. 각 finding 에 [`./review-impl/requirement-status.md`](./review-impl/requirement-status.md) 의 6-classification 라벨을 부여하고 `overbuilt` 우선 분류를 적용한다.
- 별도 9-pass 체크리스트는 수행하지 않는다. overlay 자체가 review-impl 의 delta 다.

`overbuilt` 발견 시 review-impl reference 는 보고만 산출한다 (`Decision Log` 권장 기록 + 제거 또는 문서 보강 방향 권장). 적용, 정렬, 제거는 메인 에이전트가 사용자 승인된 remediation 단계에서 수행한다 (review-only 정책, NG-2).

**auto-fix 차용 안 함**: review-impl 의 fix 경로 (auto-apply) 는 채택하지 않는다 (NG-2). 발견된 issue 는 메인 에이전트가 별도 승인 단계에서 처리하거나 다음 phase 로 deferred 기록한다.
...
| 사용자 동의 시 | Mode 갱신 (`for_action` → `for_prd`), 전환 사유를 PRD draft 또는 context 에 기록 (작성 후 master 의 `Change Log` 로 이관), [`../modes/for_prd.md`](../modes/for_prd.md) 의 P1-P5 + P6-P7 진행 (for_action 의 Step 4.5 는 skip), P8 명시 승인 게이트, P9 에서 `.claude/prds/` 에 PRD 작성 |
