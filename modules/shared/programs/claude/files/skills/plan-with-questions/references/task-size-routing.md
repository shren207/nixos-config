...
`for_prd` 모드는 PRD 규약을 따라 `.claude/prds/`에 PRD 파일을 직접 작성한다 — 본 plan-with-questions는 인터뷰·검증·자동 트리거 + PRD 작성을 모두 담당하며 별도 plan 사본은 만들지 않는다. 상세 흐름은 [`../modes/for_prd.md`](../modes/for_prd.md).
...
다음 도메인 중 2+ 동시 변경이 감지되면 후보로 표시하되, 즉시 트리거하지 않고 보조 신호 1개 이상 추가 매치를 요구한다:
...
`for_prd` 트리거 + 사용자 동의 시, 산출물은 PRD 규약을 따라 `.claude/prds/`에 직접 작성된다. plan-with-questions는 `.claude/plans/` 사본을 만들지 않고, `.claude/prds/`가 단일 SSOT다.
...
Post-Impl 5번 Final Multi-Pass Review는 `for_action` / `for_prd` 모드에서 mandatory다 ([`./post-implementation.md`](./post-implementation.md) Step 5). `for_issue` 모드에는 Post-Implementation 7단계 자체가 적용되지 않으므로 Final review도 호출되지 않는다 ([`./post-implementation.md`](./post-implementation.md) 첫 단락 SSOT). PRD/spec 산출물 부재 같은 mode-specific 조건은 일부 항목을 `N/A`로 skip할 수 있을 뿐, 단계 자체를 생략하지 않는다.
...
Phase-end 10-pass + 6-classification 동시 수행:

- [`./prd/phase-template.md`](./prd/phase-template.md) 의 Phase-End 10-pass를 그대로 수행한다. 10-pass의 10개 축은 intent, correctness, simplicity, code quality, cleanup, security, performance, validation, future-phase, PRD sync 다.
- 추가로 [`./review-impl/requirement-status.md`](./review-impl/requirement-status.md) 의 6-classification을 보조 layer로 적용한다. 6분류는 satisfied, partial, missing, conflicting, overbuilt, deferred 다.

Final review (PRD 10-pass + review-impl overlay):

- 모든 phase 완료 후 Post-Impl의 Step 5 에서 [`./prd/multi-pass-review.md`](./prd/multi-pass-review.md) 의 PRD 10-pass를 SSOT 체크리스트로 수행한다. input은 PRD master + phase 파일 + 구현 코드다.
- 결과 위에 [`./review-impl/implementation-review.md`](./review-impl/implementation-review.md) overlay를 얹는다. 각 finding에 [`./review-impl/requirement-status.md`](./review-impl/requirement-status.md) 의 6-classification 라벨을 부여하고 `overbuilt` 우선 분류를 적용한다.
- 별도 9-pass 체크리스트는 수행하지 않는다. overlay 자체가 review-impl의 delta 다.

`overbuilt` 발견 시 review-impl reference는 보고만 산출한다 (`Decision Log` 권장 기록 + 제거 또는 문서 보강 방향 권장). 적용, 정렬, 제거는 메인 에이전트가 사용자 승인된 remediation 단계에서 수행한다 (review-only 정책, NG-2).

auto-fix 차용 안 함: review-impl의 fix 경로 (auto-apply) 는 채택하지 않는다 (NG-2). 발견된 issue는 메인 에이전트가 별도 승인 단계에서 처리하거나 다음 phase로 deferred 기록한다.
...
| 사용자 동의 시 | Mode 갱신 (`for_action` → `for_prd`), 전환 사유를 PRD draft 또는 context에 기록 (작성 후 master의 `Change Log` 로 이관), [`../modes/for_prd.md`](../modes/for_prd.md) 의 P1-P5 + P6-P7 진행 (for_action의 Step 4.5 는 skip), P8 명시 승인 게이트, P9에서 `.claude/prds/` 에 PRD 작성 |
