...
| 기술 자문 (Step 3.5) | 옵션별 anchoring-neutral 평가 (`technical_matrix` + `user_facing` 두 layer + disqualifiers + evidence_gaps — 상세 schema는 [`./consulting-step.md`](./consulting-step.md) SSOT) | codex exec high reasoning |
...
5. **기술 자문 매트릭스 (Step 3.5)**: 옵션별 `technical_matrix` (메인 LLM 내부) + `user_facing` (사용자 노출 비유 layer) + disqualifiers + evidence_gaps — 상세 schema는 [`./consulting-step.md`](./consulting-step.md) SSOT
