...
- `RUNNING`: Step 5 외부 검토를 시작했으나 durable verdict가 plan 파일에 반영되지 않음. 이 상태에서는 `Change Log`에 guard-safe run token과 started-at이 있어야 한다.
- `APPLYING`: Step 5 verdict를 수신했고 Step 6 반영 중. 이 상태에서는 `Resume From=for_action.step6_da_apply`와 최신 run token에 해당하는 durable verdict summary 또는 stable artifact name이 `Change Log`에 있어야 한다.
...
- **Step 5/6 외부 검토 실행/반영**: 같은 plan 파일의 `DA State`, `Resume From`, 본문, `Decision Log`, `Change Log`를 갱신한다. 시작 직전 `RUNNING` + guard-safe run token/started-at, verdict 수신 직후 `APPLYING` + `Resume From=for_action.step6_da_apply` + 같은 run token의 durable verdict summary 또는 stable artifact name, 반영 완료 시 `CONFIRMED`/`SKIPPED`/`BLOCKED`/`NEEDS_USER` 중 하나로 기록한다. 최신 run token과 맞지 않는 늦은 verdict는 stale result로 `Change Log`에만 남기고 반영하지 않는다.
