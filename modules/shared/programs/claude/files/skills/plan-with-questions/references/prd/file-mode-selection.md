...
- 최종 master PRD 경로와 phase 파일 경로는 canonicalize 후 반드시 `.claude/prds/` 하위여야 한다.
- Split mode의 phase 파일은 `.claude/prds/prd-<feature>/phase-NN-<phase>.md` 형태만 허용한다. `NN`은 zero-padded positive integer다.
- 동일 파일 또는 디렉토리가 이미 있으면 `-2`, `-3` 같은 숫자 suffix를 slug 뒤에 붙여 collision을 해소한다. suffix 적용 후에도 같은 안전 검사를 다시 통과해야 한다.
