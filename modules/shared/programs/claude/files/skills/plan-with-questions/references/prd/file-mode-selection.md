...
- Split-file 모드에서는 master PRD와, 승인 표면에서 본문 전체가 승인된 초기 phase 파일을 **동일한 실행에서** 생성한다. Outline만 승인된 phase는 master PRD의 Phase Index에만 남기고 phase 파일은 생성하지 않는다.
- 여기서 본문 전체가 승인된 초기 phase 파일은 [`../output-templates.md`](../output-templates.md)의 full PRD approval packet 또는 approved chunk에 phase file draft body(본문 전체, durable body)가 포함된 경우만 의미한다. 요약·경로·checksum만 있는 chunk 또는 outline-only 승인은 phase 파일 생성 승인으로 간주하지 않는다.
- 상대 링크를 사용한다.
