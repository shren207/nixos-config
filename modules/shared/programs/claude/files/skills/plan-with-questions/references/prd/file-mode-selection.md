...
- Phase 파일: `.claude/prds/prd-[feature-name]/phase-01-[phase-name].md` 등. 최초 active phase 파일은 초기 승인 후 생성하고, 미래 phase 파일은 phase-start materialization gate 승인 후 생성할 수 있다.
...
- Split-file 모드에서는 master PRD와 최초 active phase 파일을 **동일한 실행에서** 생성한다. 같은 승인 packet 또는 approved chunk에 본문 전체가 포함된 추가 phase 파일도 동일 실행에서 생성할 수 있다.
- 미래 phase outline은 master PRD의 Phase Index와 `Next Phase Materialization`에 `Pending phase-start approval`로 남길 수 있다. 해당 phase 파일은 phase 시작 직전 phase-start materialization gate에서 prior phase checkpoint, 승인 후 그대로 작성될 phase file draft body(본문 전체, durable body), master PRD materialization update, phase-scoped stable step ID 범위를 승인받은 뒤 생성한다.
- 요약·경로·checksum만 있는 chunk 또는 outline-only 승인은 phase 파일 생성 승인으로 간주하지 않는다.
- 상대 링크를 사용한다.
...
- Phase 종료 시 해당 phase 파일 갱신 + master PRD 갱신 + 새로 드러난 사실이 있으면 이미 materialized 된 후속 phase 파일을 revise한다. 아직 생성되지 않은 후속 phase는 Phase Index와 phase-start materialization gate 입력을 갱신한다.
