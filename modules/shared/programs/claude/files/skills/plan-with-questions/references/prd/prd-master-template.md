...
- 경로는 `.claude/prds/...` 로 고정한다 (nixos-config의 `.claude/` 규약). upstream 원본의 `tasks/...` 경로는 본 SSOT에서 사용하지 않는다.
- `[feature-name]` slug와 split phase path의 단일 SSOT는 [`file-mode-selection.md`](./file-mode-selection.md#경로-slug-안전-규칙) 의 basename과 canonical containment 규칙이다.
