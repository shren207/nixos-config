...
- Active Phase File: N/A | Pending phase-start approval | [Phase N](./prd-[feature-name]/phase-NN-[phase-name].md) <!-- split only -->
- Next Phase Materialization: N/A | Phase N pending phase-start approval; regenerate phase file draft body from this PRD, completed phase files, and current repo context before file creation
- Next Blocking Step: N/A | <resume-state Next Blocking Step enum>
- Last Completed Step: N/A | <resume-state Next Blocking Step enum>
...
- 어떤 phase든 시작 전에 master PRD와 관련 context note를 읽는다. `Active Phase File`이 링크이면 해당 phase 파일도 읽는다. `Active Phase File` 또는 `Next Phase Materialization`이 `Pending phase-start approval`이면 존재하지 않는 phase 파일을 추측하지 말고, master PRD + 완료된 phase 파일 + 관련 context + 현재 repo 상태를 읽은 뒤 phase-start materialization gate를 수행한다. 이전 materialized phase의 `PHASE-END-COMMIT` checkpoint가 없으면 먼저 이전 phase를 닫는다.
- `Next Blocking Step`은 첫 미완료 phase/final stable step을 가리킨다. 각 step 시작 전 이 값을 현재 step으로 갱신하고, 완료 즉시 `Last Completed Step`과 다음 `Next Blocking Step`을 갱신한다.
- Split-file mode에서는 모든 phase가 완료되고 phase-end PRD sync가 커밋된 뒤 final review gate를 수행한다. follow-up commit까지 끝나 final diff가 고정된 뒤 final PR write gate를 수행한다.
...
|---|---|---|---|---|
| Phase 1: [Name] | Not Started | ... | ... | [phase-01-[name].md](./prd-[feature-name]/phase-01-[name].md) 또는 `Pending phase-start approval` |

<!-- Single-file mode: 아래 Phase Plan 섹션에 각 phase의 전체 템플릿을 둔다. Split-file mode: 위 인덱스만 남기고 상세는 별도 phase 파일로 분리한다. 아직 생성하지 않은 미래 phase는 File 칸을 `Pending phase-start approval`로 둔 뒤 phase 시작 직전 본문 전체 승인 후 링크로 교체한다. -->
...
- Split-file mode라면 master PRD의 `Phase Plan` 섹션은 제거하고 Phase Index만 남긴다. 최초 active phase 파일은 생성된 링크로 적고, 아직 materialized 되지 않은 미래 phase는 Phase Index와 `Next Phase Materialization`에 `Pending phase-start approval`로 표시한다.
- Split-file mode의 phase/final 진행 중에는 `Next Blocking Step` / `Last Completed Step`을 [`../resume-state.md`](../resume-state.md#for_prd-prd-작성-후-next-blocking-step)에 정의된 enum으로만 기록한다.
