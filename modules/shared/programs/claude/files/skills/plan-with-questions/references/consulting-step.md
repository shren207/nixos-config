...
**Durable output에 임시 dir path 기록 금지 (회귀 방지)**: 셸 호출 사이에 `CONSULT_DIR` 리터럴 값을 재사용하는 것은 runtime 요구사항이지만, plan/PRD/PR/issue/comment 같은 durable output에는 temporary scratch consult output 리터럴을 박지 않는다. 임시 경로는 세션 종료 시 사라지는 ephemeral identifier이며, durable output 적합성은 [`pinning-patterns.sh`](../../../lib/pinning-patterns.sh) helper로 검증한다. durable output에는 자문 회차 자연어 요약(예: "1차 자문(전체 N결정)")·`decision_id` list·verdict 요약만 기록한다.
