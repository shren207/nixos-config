...
본 reference가 Step 3.5 codex exec 명령의 단일 진실 원천이다. SKILL.md / for_action.md / for_issue.md는 본 섹션을 link만 하고 명령을 복제하지 않는다. (Maintainability-2 회귀 방지)

`codex-fan-out` 패턴을 차용하되 read-only sandbox + project-config 차단 + JSON 출력 강제:
...
위 schema에 맞는 JSON instance를 반환하라. schema definition은 출력하지 마라.
...
# read-only sandbox + project-config 차단 + ephemeral + high reasoning
...
- **`-C "$CONSULT_DIR"`**: cwd를 repo 외 scratch로 이동. repo의 `.codex/config.toml`(Slack/Linear MCP 등 project-scoped connector)이 자동 로드되지 않게 한다.
- **`--skip-git-repo-check`**: `-C`가 가리키는 scratch 디렉토리는 git repo 밖이라 codex가 `Not inside a trusted directory` 에러로 거부한다. 이 플래그로 그 검사를 건너뛴다 (외부 cwd 실행 시 필수 — `using-codex-exec/references/known-issues.md` 참조).
