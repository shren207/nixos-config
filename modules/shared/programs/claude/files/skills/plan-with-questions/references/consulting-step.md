...
- **`-C "$CONSULT_DIR"`**: cwd를 repo 외 scratch로 이동. repo의 `.codex/config.toml`(Slack/Linear MCP 등 project-scoped connector)이 자동 로드되지 않게 한다 (Correctness-1 회귀 방지).
