...
2. 본 reference의 "Fallback enum" 표 (정의 정본). 본 reference를 인용하는 callsite 문서(`SKILL.md`, `modes/for_action.md`, `modes/for_issue.md`, `modes/for_prd.md`, `references/output-templates.md`, `references/runtime-boundaries.md`)는 enum 문자열을 본문에 복제하지 말고 본 표를 링크로만 인용한다 — 정책 자체를 풀어 쓰는 단순 mention은 본 정책 위반이 아니다.
