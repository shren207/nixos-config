...
- **PRD status transition의 정본 owner는 `prd` 스킬.** 본 스킬은 **evidence-backed sync**로 한정한다:
  - 요청 입력에 PRD/phase 파일이 **명시적으로 포함된 경우에 한해서만** `.claude/prds/prd-*.md`의 체크박스 / validation notes / change log를 갱신한다.
...
- risk에 맞는 최소 충분 validation을 고른다: static check, unit test, integration test, API-level E2E, browser/UI check, simulator check, screenshot, manual smoke, observability check. 선택 가이드는 [`../prd/references/validation-paths.md`](../prd/references/validation-paths.md)를 따른다.
