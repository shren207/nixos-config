...
- **참고 레퍼런스 식별자**: 관련 PR/이슈/외부 링크는 안정 식별자(URL, `owner/repo#N`, 또는 머지된 commit SHA)를 사용한다. 같은 repo의 bare `#N`은 Summary의 leading standalone GitHub closing-keyword line(`Closes #N`)에만 사용한다. 본인 PR의 mid-flight commit hash 또는 squash 전 partial hash chain은 박제 금지 — squash 후 dangling 위험. "관련 PR 참조" 식의 모호한 레퍼런스도 금지.
