...
| # | 섹션 | 역할 |
|---|------|------|
| 1 | Summary | 핵심 변경 1-3 bullet, 마지막에 standalone leading `Closes #N` line |
| 2 | 기존 문제/배경 | Pain point — 왜 이 변경이 필요한지 |
| 3 | CIR (Change Intent Record) | 발견 경위 + 설계 의도 + 대안 검토 이력. **리뷰 실행 식별자/partial hash chain 포함 금지** (변경 의도는 자연어로) |
| 4 | ADR (Architecture Decision Record) | 대안 비교 테이블 (대안/설명/장점/단점/결정) |
| 5 | 구현 상세 | 변경 파일 테이블 + 핵심 코드 스니펫 |
| 6 | 참고 레퍼런스 | 검증에 필요한 외부 링크 또는 full merged 40-char SHA (partial hash chain 금지) |
...
2. **연관 이슈 탐색**: 커밋 메시지, 브랜치명, 변경 내용에서 이슈 번호를 추출한다. 관련 이슈가 있으면 Summary 섹션 말미에 standalone leading `Closes #N` line으로만 포함한다.
...
- **참고 레퍼런스 식별자**: 검증에 필요한 외부 링크 또는 full merged 40-char SHA만 사용한다. 같은 repo의 bare 이슈 번호는 Summary의 leading standalone GitHub closing-keyword line에만 사용한다. 본인 PR의 mid-flight commit hash 또는 squash 전 partial hash chain은 박제 금지 — squash 후 dangling 위험. "관련 문서 참조" 식의 모호한 레퍼런스도 금지.
...
- **PR 본문 박제 금지 항목**: 리뷰 실행 식별자, partial commit hash chain, 워크트리 절대경로.
