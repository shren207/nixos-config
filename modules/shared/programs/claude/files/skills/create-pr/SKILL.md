...
4. `create` mode는 명시적 보수 정책으로 repo-local head만 지원한다. cross-repo/fork head는 user-owned fork 일부를 `gh pr create --head <user>:<branch>`로 표현할 수 있더라도 repo identity와 owner type에 따른 예외가 있으므로 fail closed 처리한다. repo identity를 명시적으로 검증하는 별도 API-backed 절차가 생기기 전에는 `apply-approved create`에서 지원하지 않는다.
