...
| `PARENT_NUM` | parent의 GitHub visible issue number (integer). REST path의 `{issue_number}` | `<parent-number>` |
| `ISSUE_URL` | `gh issue create`가 반환하는 HTML URL | `https://github.com/OWNER/REPO/issues/<issue-number>` |
| `ISSUE_NUM` | `ISSUE_URL`에서 추출한 child의 visible number | `<issue-number>` |
| `ISSUE_ID` | 새로 생성된 child 이슈의 database id. Sub-Issues POST body의 `sub_issue_id` | `<database-id>` |
...
2. anchored URL: `^https://github\.com/([^/]+)/([^/]+)/issues/([0-9]+)/?([?#].*)?$`
   - suffix는 선택적 trailing slash(`/?`)와 query/fragment(`[?#].*`)만 허용한다. `/issues/<number>/evil` 같은 추가 path segment는 거부된다.
...
| `"제목" --parent <parent-number>` | `<parent-number>` | `"제목"` | 숫자 값 매칭 |
| `--parent=<parent-number> "제목"` | `<parent-number>` | `"제목"` | `--parent=` 형식 |
| `"제목" --parent https://github.com/OWNER/REPO/issues/<parent-number>` | `<parent-number>` | `"제목"` | URL 값 매칭 (same-repo) |
| `--parent <invalid-number> "제목"` | — | — | exit 1 (값 패턴 불일치, 오타 방지) |
| `"제목" --parent` | — | — | exit 1 (값 누락) |
| `--parent <number-a> --parent <number-b> "제목"` | — | — | exit 1 (중복 지정) |
| `-- "--parent 문서화 이슈"` | (unset) | `"--parent 문서화 이슈"` | escape로 literal 보존 |
| `"제목" -- --parent <parent-number>` | (unset) | `"제목" --parent <parent-number>` | standalone `--` 이후는 옵션 검색 제외 |
...
if ! PARENT_META=$(gh api "/repos/$OWNER/$REPO/issues/$PARENT_NUM" \
     ...
     || [ -z "$PARENT_META" ]; then
  echo "ERROR: parent ${OWNER}/${REPO}#${PARENT_NUM} 조회 실패, 이슈가 아님, 또는 PR 번호"
  exit 1
fi
```
...
- **References**: 비자명 주장의 출처 링크 최소 1개 이상 (체크리스트 B1/B4). 공식 docs URL, repo 내부 파일 경로(`path/to/file.nix:<line>`), 필요한 경우의 이슈/PR URL placeholder, 또는 full merged 40-char SHA만 사용한다. 관련 이슈/PR 번호 인용은 출처 입증에 불가결한 경우에만 사용 (Step 1-b 검색 결과 자동 첨부 금지 — close/rename 시 stale). 근거 부재 시 `[UNVERIFIED]` 라벨로 대체.
...
if [ -n "$PARENT_NUM" ]; then
  ...
  if [ -z "$ISSUE_ID" ] || ! [[ "$ISSUE_ID" =~ ^[0-9]+$ ]]; then
    ...
    if gh api -X POST "/repos/$OWNER/$REPO/issues/$PARENT_NUM/sub_issues" \
      ...
      echo "SUBISSUE_LINKED=${OWNER}/${REPO}#${ISSUE_NUM} -> parent ${OWNER}/${REPO}#${PARENT_NUM}"
    else
