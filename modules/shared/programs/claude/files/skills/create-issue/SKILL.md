...
- **References**: 비자명 주장의 출처 링크 최소 1개 이상 (체크리스트 B1/B4). 공식 docs URL, repo 내부 파일 경로(`path/to/file.nix:LINE`), 관련 이슈/PR의 URL 또는 `owner/repo#N`, 또는 full merged 40-char SHA만 사용한다. 관련 이슈/PR 번호 인용은 출처 입증에 불가결한 경우에만 사용 (Step 1-b 검색 결과 자동 첨부 금지 — close/rename 시 stale). 근거 부재 시 `[UNVERIFIED]` 라벨로 대체.
