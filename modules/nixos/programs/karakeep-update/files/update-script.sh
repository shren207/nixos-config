...
if [ -n "$CURRENT_DIGEST" ] && [ "$CURRENT_DIGEST" = "$NEW_IMAGE_DIGEST" ]; then
  ...
  if [ -n "$LATEST_VERSION" ] && [ "$CURRENT_TAG_CLEAN" != "$LATEST_VERSION" ]; then
    ...
    echo "Update modules/nixos/programs/docker/karakeep.nix tag, then run nrs."
  fi
...
if $BRIDGE_GUARD_WARN; then
  send_notification "$SERVICE_DISPLAY_NAME Update" "업데이트 완료(주의): ${VERSION_INFO} 적용됨. bridge 연계 서비스 상태 확인 필요" 0
else
