...
flock -n 200 || { echo "ERROR: Another uptime-kuma-update is already running"; exit 1; }
...
# ERR trap — 컨테이너 자동 복구 (모니터링 서비스라 복구 필수)
cleanup() {
  local recovery_msg=""
...
if $DRY_RUN; then
  ...
  echo "  4. Backup SQLite ($DATA_DIR/kuma.db)"
...
if [ -n "$CURRENT_DIGEST" ] && [ "$CURRENT_DIGEST" = "$NEW_IMAGE_DIGEST" ]; then
  ...
  if [ -n "$LATEST_VERSION" ] && [ "$CURRENT_TAG_CLEAN" != "$LATEST_VERSION" ]; then
    ...
    echo "Update modules/nixos/programs/docker/uptime-kuma.nix tag, then run nrs."
  fi
...
# ─── 4. SQLite 백업 (컨테이너 정지 상태에서 깨끗한 백업) ─────────
echo ""
echo "Backing up database..."
umask 077  # kuma.db에 비밀번호 해시 포함되므로 umask 필수
TIMESTAMP=$(date +%Y%m%d-%H%M%S)
BACKUP_FILE="$BACKUP_DIR/kuma-${TIMESTAMP}.db.gz"
gzip -c "$DATA_DIR/kuma.db" > "$BACKUP_FILE"

# 백업 무결성/크기 검증
if ! gzip -t "$BACKUP_FILE" 2>/dev/null; then
  echo "ERROR: Backup file is corrupted"
  send_notification "$SERVICE_DISPLAY_NAME Update" "업데이트 중단: 백업 파일 손상" 1
...
if [ "$BACKUP_SIZE" -lt 1024 ]; then
  ...
  send_notification "$SERVICE_DISPLAY_NAME Update" "업데이트 중단: 백업이 비어있음" 1
  echo "Restarting container..."
  systemctl start "$SERVICE_UNIT"
  exit 1
fi
...
done < <(find "$BACKUP_DIR" -name "kuma-*.db.gz" -mtime +7 2>/dev/null)
