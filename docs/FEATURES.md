...
| 상태 | 아이콘 | 설명 | 삭제 방식 |
|------|--------|------|----------|
| gone | ✅ | 원격에서 삭제된 브랜치 (PR 머지 후 삭제됨) | `-D` (강제) |
| stale | ⚠️ | 30일 이상 된 로컬 전용 브랜치 | `-D` (강제) |
| protected | 🔒 | 보호 브랜치 (main, master, develop, stage) | 삭제 불가 |
| current | 📍 | 현재 체크아웃된 브랜치 | 삭제 불가 |
...
── gone (3개) ──
  ✅ [gone] feat/login (origin/feat/login)
  ✅ [gone] fix/bug-123 (origin/fix/bug-123)
  ✅ [gone] JIRA-456 (origin/JIRA-456)

── stale (2개) ──
  ⚠️ [stale] experiment/test (45일 경과)
  ⚠️ [stale] old-feature (120일 경과)
