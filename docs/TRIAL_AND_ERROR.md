...
> ⚠️ **운영 참고**: 이 문서는 반복되는 실패 모드와 대응 기준을 기록합니다. 최신 정보는 각 SKILL.md와 모듈별 문서를 참조하세요.
...
- **의도**: rip만 제거
- **결과**: 4개 inputs 전부 업데이트 (nixpkgs, home-manager, nix-darwin, nix-vscode-extensions — pre-Zed migration; 현재는 `modules/darwin/programs/zed/` 참조)
...
**생성했던 파일 구조:** (현재 경로는 `modules/nixos/programs/anki-*`)
...
**daemon sync** (`atuin-daemon/src/server/sync.rs`):
```rust
...
| history end (auto_sync) | ✅ 호출 | `crates/atuin/src/command/client/history.rs` | 터미널 명령 실행 후 |
| daemon sync | ✅ 호출 | `atuin-daemon/src/server/sync.rs` | daemon 내부 스케줄러 |
...
**소스코드 증거** (`crates/atuin/src/command/client/history.rs`):
...
**`should_sync()` 함수** (`crates/atuin-client/src/settings.rs`):
...
**운영 기준**: 문제의 근본 원인을 파악하지 않고 증상만 치료하면 시스템이 불필요하게 복잡해진다.
...
**운영 기준**: 새로운 기능을 추가하기 전에 기존 도구가 이미 제공하는 기능을 확인하라.
...
**운영 기준**: 복잡한 해결책은 대개 문제를 제대로 이해하지 못했다는 신호다.
...
**운영 기준**: 도구의 정확한 동작을 알고 싶다면 소스코드를 읽어라.
