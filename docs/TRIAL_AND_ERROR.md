...
- **의도**: rip만 제거
- **결과**: 4개 inputs 전부 업데이트 (nixpkgs, home-manager, nix-darwin, nix-vscode-extensions — 당시 VSCode 시점. Zed migration #329 후 일시 제거됐다가 #329 reverse PR로 다시 활성: 현재 에디터 모듈은 `modules/darwin/programs/vscode/`)
...
>
> **Note**: 이 시행착오의 대상 프로그램(Cursor)은 #171에서 VSCode로 대체된 뒤 #329에서 Zed로 마이그레이션됐고, 다시 #329 reverse PR로 VSCode가 복귀했다. 현재 에디터 모듈은 `modules/darwin/programs/vscode/`.
