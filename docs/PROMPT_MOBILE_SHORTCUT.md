...
> preset 이름을 single-quote로 감싸 shell injection 방어.
...
[If] okValue == true:
  [Get Dictionary Value] key="rendered"
  [Copy to Clipboard]
  [Show Notification] "✓ selectedPreset 복사 완료"
[Otherwise]:
...
[Repeat with Each Item] in missingVars:
  ...
  [Replace Text] "'" with "'\\''" in inputValue  ← single-quote escaping
  varArgs = varArgs + " --var '" + Repeat Item + "=" + escapedValue + "'"
[End Repeat]

[Run Script over SSH]
  Script: ...PATH설정...; prompt-render --preset 'selectedPreset' varArgs --non-interactive --format json --stdout-only
