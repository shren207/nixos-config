...
prompt-render --preset feature-dev-full \
  ...
  --var 'DA_MODEL_2=claude-opus-4-6' \
  --var 'DA_INTENSITY=strict' \
  --var 'DA_TIMING=both' \
...
> 최종 명령 예시:
> ```bash
> prompt-render --preset "$(printf '%s' 'feature-dev-full' | tr -d '\n\r')" --var 'DA_TOOL=codex exec' --var 'DA_MODEL_1=gpt-5.4' --var 'DA_MODEL_2=claude-opus-4-6' --var 'DA_INTENSITY=strict' --var 'DA_TIMING=both' --non-interactive --format json --stdout-only
> ```
...
→ "DA 강도"       — [light / standard / strict] 중 선택
→ "DA 시점"       — [plan / post-impl / both] 중 선택
→ 클립보드에 복사
