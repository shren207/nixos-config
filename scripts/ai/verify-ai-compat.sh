...
require_agents_override_generated_text() {
  ...
  if ! grep -qxF "$start_marker" "$path" || ! grep -qxF "$end_marker" "$path"; then
    fail "$desc marker 없음: AGENTS.override.md"
    return
  fi
...
require_contract_text \
  ...
  '$parallel-audit` invocation is auditor-bounded internal native subagent fan-out explicit delegation' \
  "parallel-audit bounded native fan-out authorization"
