...
# - 우회: SKIP_PINNING_CHECK=1 (또는 true)로 전체 skip.
set -euo pipefail
...
# 패턴 B — DA finding ID prefix 확장: 약어 + Title Case bundle + 8 세부 도메인 모두 매치.
# DA 출력 형식 단일 진실 원천: modules/.../skills/run-da/references/da-domains.md
PATTERN_B='\b(Correctness|Design|Regression|Maintainability|SECURITY|HALLUCINATION|SIDE_EFFECT|CONSISTENCY|READABILITY|CLEAN_CODE|YAGNI|NGMI|CORR|REGR|DESIGN|MAINT|REG|CIR|F)-[A-Za-z0-9-]+\b'

# 패턴 C — DA 키워드 컨텍스트: "DA Round N" / "DA for_pr" / "DA for_plan" / "DA 피드백".
# 앞쪽 단어경계 + Round 다음 숫자 의무화로 좁힘.
PATTERN_C='\bDA (Round [0-9]+|for_pr|for_plan|피드백)\b'

# 패턴 D — 백틱 안 partial hex (HASH_MIN ~ HASH_MAX 자리). commit msg 첫 줄이 revert로 시작하면 skip.
PATTERN_D='`[a-f0-9]{'"$HASH_MIN"','"$HASH_MAX"'}`'
...
if echo "$MSG_BODY" | grep -qE "$PATTERN_C"; then
  warn "DA 키워드 박제 감지. 검토 라운드/모드 표기는 commit message가 아니라 PR description 또는 별도 노트에 둬라."
  echo "$MSG_BODY" | grep -nE "$PATTERN_C" | sed 's/^/         /' >&2
  found=1
fi

if [[ ! "$FIRST_LINE" =~ ^[Rr]evert ]]; then
  if echo "$MSG_BODY" | grep -qE "$PATTERN_D"; then
    warn "Partial commit hash 박제 감지. squash 머지 시 dangling 위험. 안정 식별자(PR 번호, 머지된 SHA)로 대체하라."
    echo "$MSG_BODY" | grep -nE "$PATTERN_D" | sed 's/^/         /' >&2
    found=1
  fi
fi

if [ "$found" -eq 1 ]; then
  warn "위 경고는 차단하지 않습니다 (warn-only). 검토 후 amend로 정정하거나 의도적 사용이면 무시하세요. SKIP_PINNING_CHECK=1로 전체 skip 가능."
fi
