...
- [ ] `plan-with-questions/SKILL.md` description Edit — 흡수 trigger 추가 (PRD 작성, 구현 감사, 스펙 대비 감사, overbuilt 검사, Living PRD, phase 계획, PRD 업데이트, 문서 대비 구현 리뷰 등 12개).
- [ ] `plan-with-questions/SKILL.md` 빠른 참조 표 — 흡수 후 references 위치 갱신 (`./references/validation-paths.md`, `./references/prd/*`, `./references/review-impl/*`).
- [ ] `plan-with-questions/SKILL.md` 모드 판별 표 — for_prd 트리거 키워드 보강 (PRD 작성·구현 감사 등 자연어 입력 자동 매칭).
- [ ] `plan-with-questions/SKILL.md` 차용 reference 표 — `../prd/...`, `../review-implementation/...` 제거, 새 위치 명시.
