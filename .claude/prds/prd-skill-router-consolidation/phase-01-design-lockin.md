...
- [x] `cat scripts/ai/verify-ai-compat.sh` — `EXPECTED_EXPOSED` 배열 정확한 위치 **line 349 시작 ~ line 362 닫는 `)`**. `prd` = **line 357**, `review-implementation` = **line 358** 잔존 확인. (PRD master FR-7 명시 "349-357"은 prd/review-impl entry 라인까지의 sub-range; 배열 전체 범위는 349-362.)
...
- [x] scripts/ai/verify-ai-compat.sh의 EXPECTED_EXPOSED 배열 정확한 라인 + 형식 baseline — Discoveries에 line 349-362 (prd=357, review-impl=358) 기록.
...
| `verify-ai-compat.sh` EXPECTED_EXPOSED | line 349-362 (12 entries + 닫는 `)`), prd=**line 357**, review-implementation=**line 358** | PRD FR-7 인용 "349-357"은 prd entry까지 sub-range; Phase 4 Edit 시 line 357-358 두 entry 제거 |
...
- FR-7 → **line 349-362** (배열 시작 + 12 entries + 닫는 `)`); 두 entry 제거 대상은 line 357-358
