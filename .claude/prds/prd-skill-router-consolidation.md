...
### DL-19: 9-pass review-only 절차를 implementation-review.md 신규 reference로 이관 (Post-Impl 3 R2)
- **Status**: accepted
...
- **Decision**: 사용자가 옵션 α 채택. `plan-with-questions/references/review-impl/implementation-review.md` 신규 파일에 deleted SKILL.md:56-68의 9-pass 본문 이관. `requirement-status.md`는 6-classification taxonomy 전용 유지. 6곳의 9-pass owner link를 새 파일로 갱신 (SKILL.md, modes/for_prd.md, modes/for_issue.md, references/output-templates.md, references/task-size-routing.md, references/post-implementation.md). 6-classification ≠ 9-pass 축 분리 명시 (PRD Final 10-pass와도 다른 축).
- **Consequences**: review-implementation skill의 review-only 능력이 새 owner(`plan-with-questions`)로 추적 가능하게 이관됨. Phase 5 closure의 9-pass 주장이 실재 절차에 대응. DL-9 (auto-fix 폐기)와 NG-2 (auto-fix 미채택)는 그대로 유지 (본 reference도 review-only 명시). 추가로 Round 2 자동 수정 5건 (C-2/R-2/M-1 PRD 템플릿 stale prd 경로, C-3 fix mode action 잔존, R-3/M-2 review-implementation 잔존, M-3 validation path 수 하드코딩) 별도 commit으로 반영. Round 2 D-2 (issue body routing intent marker)는 PRD f7ef153 + DL-18 결정과 충돌 → NOT_AN_ISSUE 정리 (별도 DL 기록 불필요).
