...
- **Skill context budget warning**: `codex exec` subprocess에서 `Warning: Exceeded skills context budget`가 보이면 installed plugin 또는 user-global 스킬 설명까지 로드된 것이다. 감사/리뷰처럼 plugin 도구가 필요 없는 실행은 scratch cwd, scratch `CODEX_HOME`(기존 `auth.json`만 복사), `--ignore-user-config`, `--disable plugins`, `--ephemeral`을 함께 쓰고, 대화형 세션의 plugin 설정은 유지한다.
...
Codex CLI는 디렉토리 심링크를 `follow_links(true)`로 순회한다.
