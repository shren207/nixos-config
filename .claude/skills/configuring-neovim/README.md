...
```
Ctrl+Space   선택 확장    단어 → 표현식 → 블록 → 함수 → ...
Backspace    선택 축소    (Visual 모드에서) 반대 방향으로 축소
```

> **실전**: 커서를 변수 위에 두고 `Ctrl+Space` 연타하면 점점 넓은 범위가 선택됨. 원하는 범위에서 `y` (복사), `d` (삭제), `c` (변경) 등 실행.
