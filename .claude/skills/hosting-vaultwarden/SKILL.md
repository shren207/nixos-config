...
- Tailscale VPN 내부에서 `https://vaultwarden.greenhead.dev` 도달
- agenix가 호스트의 identity key(`/home/<user>/.ssh/id_ed25519`)로 복호화한 secret 파일(`/run/agenix/vaultwarden-admin-token`) 접근 권한
- tmpfs 마운트 권한 (`/run/vaultwarden-env`)
- Bitwarden 클라이언트 (Mac 브라우저 확장 / iPhone 앱)는 본 스킬의 자동화 대상 외 — GUI 전제
