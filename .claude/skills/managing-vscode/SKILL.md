...
├── islands-dark.nix            # Islands Dark custom pinned source + VSCode CSS patch
└── files/
...
확장 소스 선택: 먼저 `open-vsx`에서 검색, 없으면 `vscode-marketplace` 사용. Marketplace/Open-VSX에 없거나 upstream source patch/font/CSS까지 함께 고정해야 하면 custom pinned source 경로를 사용한다.
