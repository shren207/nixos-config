...
**pre-push**:
- `tomlkit-fixtures` — shared tomlkit runtime에서 shell script fixture와 Codex hook deterministic fixture를 함께 실행 ([`tests/run-tomlkit-pre-push-tests.sh`](./tests/run-tomlkit-pre-push-tests.sh))
- `pinning-rules` — pinning rules/hooks/workflow regression 테스트 ([`tests/test-pinning-rules.sh`](./tests/test-pinning-rules.sh))
